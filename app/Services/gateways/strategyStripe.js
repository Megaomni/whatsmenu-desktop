...
class StrategyStripe {
    ...
    async addSubscriptionDiscount(data) {
        const discount = await this.stripe.customers.createBalanceTransaction(
            data.customerId,
            {
                amount: data.value,
    ...
    async addSubscriptionIncrement(data) {
        const increment = await this.stripe.customers.createBalanceTransaction(
            data.customerId,
            {
                ...
                switch (data.billing_reason) {
                    ...
                    case "subscription_cycle":
                        ...
                        try {
                            ...
                            const upcomingInvoice = await user.invoices()
                                .where({ status: "upcoming", expiration }).last()
