...
addEventListener('message', ({ data }) => {
  const { context, packageHours } = JSON.parse(data)
  ...
  while (dayCounter <= context.profile.options.package.distanceDays.end) {
    ...
    if (weekday.length) {
      packageHours[date.toFormat('dd-MM-yyyy')] = weekday.map((period) => {
        const periodResult = []
        ...
        if (now.toFormat('dd-MM-yyyy') === openDate.toFormat('dd-MM-yyyy')) {
  ...
  const packageCalendar = calendarDate.toFormat('dd-MM-yyyy')
