...
export class TableOpened {
  ...
  constructor(tableOpened: TableOpenedType) {
    ...
    this.commands = tableOpened.commands
      ? tableOpened.commands.map((command) => new Command(command))
      : []
    this.fees = tableOpened.fees
    this.formsPayment = tableOpened.formsPayment
    this.perm = DateTime.fromSQL(tableOpened.updated_at)
      .diff(DateTime.fromSQL(tableOpened.created_at), [
        'months',
        'days',
        'hours',
        'minutes',
  ...
  public getUpdatedFees = (filter = false, all = false) => {
    const fees = (
      filter ? this.commands.filter((c) => c.status) : this.commands
    ).reduce((fees: ProfileFee[], command) => {
  ...
  public getTotalValue = (
    ...
    const tableTotal = this.commands.reduce(
      (result, command) =>
        Number(
          Math.fround(result + command.getTotalValue('command')).toFixed(2)
        ),
      0
    )
    const feeTotal = this.getUpdatedFees(false, true).reduce(
      (feeTotal, fee) => {
    ...
    const formsPaymentTotal = this.formsPayment.reduce(
      (formsPaymentTotal, formPayment) =>
        Number(Math.fround(formsPaymentTotal + formPayment.value).toFixed(2)),
      0
    )
    ...
    switch (only) {
      ...
      case 'lack':
        return Number(
          Math.fround(
            Math.max(tableTotal + feeTotal - formsPaymentTotal - value, 0)
          ).toFixed(2)
        )
      case 'paid':
