...
export default class Cart {
  ...
  public returnMaskedContact() {
    switch (i18n.language) {
      case 'pt-BR': // Formatação para números brasileiros
        if (this.type === 'T') {
          return '-'
        }
        if (this.client) {
      ...
      case 'en-US': // Formatação para números dos Estados Unidos
        if (this.type === 'T') {
          return '-'
        }
        if (this.client) {
      ...
      default:
        return this.client.whatsapp // Retorna o número sem formatação se o idioma não for suportado
    }
  }
