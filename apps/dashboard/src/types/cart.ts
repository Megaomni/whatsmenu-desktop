...
import Profile, { ProfileFormPayment, ProfileOptions, ProfileType } from './profile'
...
export default class Cart {
  ...
  constructor({
    ...
    taxIfood,
    orderId,
  }: CartType & { origin?: 'whatsmenu' | 'portal' | 'ifood' }) {
    this.id = id
    ...
    this.formsPayment.forEach((formPayment) => {
      if (formPayment.payment === 'cashback') {
        formPayment.value = Math.min(this.getTotalValue('total'), formPayment.value)
      }
    })
  }
  ...
  public getTotalValue(sumWith: 'subtotal' | 'total' | 'lack' | 'addon' | 'paid') {
    ...
    switch (sumWith) {
      ...
      case 'total':
        ...
        return total + this.taxDelivery - cupomValue + addonTotal
      case 'lack':
        return Number(
          Math.fround(
            total +
  ...
  static fromIfood({ order, profile }: { order: IfoodOrderType; profile: Profile | ProfileType }): Cart {
    ...
    let cupom
    let status
    if (order.orderType === 'DELIVERY' || order.orderType === 'TAKEOUT' || order.orderType === 'INDOOR') {
        type = 'D'
    }
    ...
    const ifoodCart: CartType = {
      ...
      clientId: 999,
      secretNumber: order.customer.documentNumber ? order.customer.documentNumber : null,
      status: status as CartType['status'],
      type: type as CartType['type'],
      taxDelivery: order.total.deliveryFee,
      timeDelivery: order.delivery.deliveryDateTime,
      formsPayment: order.payments.map((payment) => ({
        flag: payment.methods[0].card.brand,
        change: payment.methods[0].cash ? payment.methods[0].cash.changeFor : null,
        label: payment.methods[0].method,
        value: payment.prepaid,
        payment: payment.methods[0].method,
      } as CartType['formsPayment'][0])),
      print: 0,
      tentatives: 0,
      total: order.total.subTotal,
      controls: {},
      statusPayment: order.payments[0].type,
      packageDate: order.preparationStartDateTime,
      created_at: DateTime.fromISO(order.createdAt).toSQL(),
      updated_at: order.updatedAt,
      itens: order.itens.map((item) => ({
        cartId: order.id,
        controls: {},
        created_at: DateTime.fromISO(item.createdAt).toSQL(),
        updated_at: DateTime.fromISO(item.updatedAt).toSQL(),
        id: item.id,
        name: item.name,
