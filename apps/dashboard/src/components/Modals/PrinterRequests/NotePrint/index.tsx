  ...
  const getFormsPaymentToPrint = (noCashback = true) => {
    ...
    switch (printType) {
      ...
      case 'command':
        ...
        break
      default:
        formsPayment = (cart.statusPayment === 'offline' ? cart?.formsPayment : cart?.formsPayment.filter(f => f.paid || f.payment === 'cashback')) ?? []
        if (noCashback) {
          formsPayment = formsPayment.filter((formPayment) => formPayment.payment !== 'cashback')
        }
        break
    }
    return formsPayment
      ...
      <>
        ...
        {complement.itens?.map((complementItem, index) => {
          const complementItemTotal =
            complementItem.value > 0 ? `(${currency({ value: complementItem.value * (complementItem.quantity || 1) * (item.details.value > 0 ? 1 : item.quantity), withoutSymbol: true })})` : ''
          return (
            <Print.Row
            ...
            <>
              ...
              {cartItem.details.flavors?.map((flavor, index) => (
                <Fragment key={`${flavor.code}-${index}`}>
                  ...
                  {flavor.complements?.map((complement) => complementLayout(complement, cartItem))}
                  {flavor.implementations?.map((implementatiton) => implementationLayout(implementatiton))}
                </Fragment>
              ))}
            </>
          )}
          {(cartItem.type === 'default' || !profile.options.pizza.multipleComplements) && cartItem.details.complements?.map((complement) => complementLayout(complement, cartItem))}
    ...
    >
      ...
      <>
        ...
        {cart.formsPayment && cart.formsPayment[0]?.addon?.status && (
          <Print.Row left={`${addonTypeLabel()}:`} right={`${cart.formsPayment[0]?.addon.type === 'fee' ? '+' : '-'}${currency({ value: cart.getTotalValue('addon'), withoutSymbol: true })}`} />
        )}
        {cart.cupom && <Print.Row left={`Cupom:`} right={`-${getCupomValue().cupomDisplayValue}`} />}
        {getFormsPaymentToPrint(false).some((formPayment) => formPayment.payment === 'cashback') && <Print.Row left={`Cashback:`} right={`-${currency({ value: cart.getTotalValue('cashback'), withoutSymbol: true })}`} />}
