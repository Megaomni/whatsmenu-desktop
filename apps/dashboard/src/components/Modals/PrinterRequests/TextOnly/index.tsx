...
export function TextOnly({ carts, componentRef, autoPrint, report, titleTable, command, table, opened, ...props }: PropsType) {
  const { t } = useTranslation()
  const { profile: profileContext, getBartender, user, currency } = useContext(AppContext)
  const cart = carts[0]
  ...
  const underlineSeparator = new Array(props.paperSize === 58 ? 32 : 48).fill('-').join('')

  const line = new Array(props.paperSize === 58 ? 32 : 48).fill('\u00A0').join('')

  subTotal = carts.reduce((subTotal, c) => subTotal + c.getTotalValue('subtotal'), 0)
            ...
            <div key={hash()}>
              ...
              <div>
                ...
                {prod.obs ? (
                  <p className="m-0">
                    {' '}
                    <span className="fw-boldtest">Obs: </span> {prod.obs} {props.appPrint ? '\n' : null}
                  </p>
                ) : null}
                <div className="m-0">
                  {'\u2000\u2000\u2000'}
    ...
    <>
          ...
          return (
            <div key={cart.id}>
              {index === 0 ? (
                <>
                  <div key={cart.id}>
                    {DateTime.fromSQL(created_at, { zone: profile.timeZone }).toFormat(`${t('date_format')} HH:mm:ss`)}
                    {props.appPrint ? '\n' : null}
                  </div>
                  {props.type !== 'command' && props.type !== 'table' && (
                    <div>
                      ...
                      <span className="fw-boldtest">{cart.status === 'canceled' ? ` ${t('cancelled_up')}` : null}</span>
                      {props.appPrint ? '\n' : null}
                    </div>
                  )}
                  <div>
                  ...
                  {props.type !== 'table' && (
                    <div>
                      <span className="fw-boldtest">{props.type === 'command' || props.type === 'T' ? t('order_slip') : t('client')}:&nbsp;</span>
                  ...
                  {cart.type === 'T' && props.type !== 'command' && props.type !== 'table' && bartender && (
                    <div>
                      <span className="fw-boldtest">{t('waiter')}:&nbsp;</span>
                      <span>
                  ...
                  {cart.type === 'P' && (
                    <div>
                      <div>
                        <span className="fw-boldtest">{t('delivery_date')}: </span>
              ...
              (props.type === 'command' ? command?.fees : (opened ?? table?.opened)?.getUpdatedFees(!report, true))
                ...
                .map((fee, index, arr) => {
                  return fee.status && fee.automatic ? (
                    <div key={fee.code}>
                      <div className={`${index === 0 && 'pt-1'} ${index === arr.length - 1 && 'pb-1'}`}>
                        <span>{fee.code}</span>
                        <span>
                          {rightText(
                            currency({
              ...
              .map((commandMap, index, arr) => {
                return (
                  <div key={commandMap.code}>
                    <div className={`${index === 0 && 'pt-1'} ${index === arr.length - 1 && 'pb-1'}`}>
                      <span>{!commandMap.status ? `${commandMap.name}\u2000\u2000\u2000(PAGO)` : commandMap.name}</span>
                      <span>
                        {rightText(
                          currency({
                          ...
                          !commandMap.status ? `${commandMap.name}\u2000\u2000\u2000(PAGO)` : commandMap.name
                        )}
                      </span>
                    </div>
                  </div>
      ...
      <div>
        ...
        <div className="pt-1">
          ...
          <span>{rightText(currency({ value: subTotal, withoutSymbol: true }), 'Sub-Total:')}</span>
          {props.appPrint ? '\n' : null}
        </div>
        {carts[0]?.address && carts[0].cupom?.type !== 'freight' && (
          <div>
            ...
            <span>
              {rightText(carts[0].taxDelivery > 0 ? currency({ value: carts[0].taxDelivery, withoutSymbol: true }) : t('free'), t('delivery_fee'))}
            </span>
            {props.appPrint ? '\n' : null}
          </div>
        )}
        ...
        <div>
          ...
          <span>{rightText(currency({ value: total, withoutSymbol: true }), 'Total:')}</span>
          {props.appPrint ? '\n' : null}
        </div>
        ...
        {transshipment > 0 ? (
          <>
            <div>
              ...
              <span>{rightText(currency({ value: transshipment, withoutSymbol: true }), `${t('change_for')}:`)}</span>
              {props.appPrint ? '\n' : null}
            </div>
            <div>
              <span className="fw-boldtest">
              ...
              <span>{rightText(currency({ value: Math.max(transshipment - total, 0), withoutSymbol: true }), `${t('change')}:`)}</span>
              {props.appPrint ? '\n' : null}
            </div>
          </>
        ) : null}
        ...
        {carts[0]?.type === 'T' && (props.type === 'command' || props.type === 'table') && (
          <div>
            <div>
              ...
              <span>{rightText(currency({ value: paid, withoutSymbol: true }), t('paid'))}</span>
              {props.appPrint ? '\n' : null}
            </div>
            {(props.type === 'command' ? command?.formsPayment : (opened ?? table?.opened)?.formsPayment)?.map((formPayment) => (
              <p className="m-0" key={hash()}>
                <span className="fw-boldtest">{`\u2000\u2000Em ${formPayment.label}`}</span>
                <span>
                  {rightText(currency({ value: formPayment.value, withoutSymbol: true }), '\u2000\u2000Em\u2000\u2000\u2000\u2000\u2000\u2000')}
                  {formPayment.change ? (
                    <p className="m-0">
                      <span>{`\u2000\u2000Troco`}</span>
                      {rightText(
                        currency({ value: formPayment.change - formPayment.value, withoutSymbol: true }),
                        '\u2000\u2000Troco\u2000\u2000\u2000\u2000\u2000\u2000'
                      )}
                    </p>
                  ) : null}
        ...
        {carts[0]?.type === 'T' && (props.type === 'command' || props.type === 'table') && (
          <div>
            ...
            <span>{rightText(currency({ value: lack, withoutSymbol: true }), 'Faltam:')}</span>
          </div>
        )}
        {carts[0]?.type !== 'T' && (
          <div>
        ...
        <>
          ...
          <div>
            ...
            <span className="fs-7 fw-boldtest  w-100">
              {carts && centerText(carts[0]?.typeDeliveryText(textPackage(profile.options.package.label2), true))}
            </span>
            {props.appPrint ? '\n' : null}
          </div>
          <div className="fs-7 fw-boldtest  w-100">
            {carts[0]?.type === 'P' ? centerText(`**${textPackage(profile.options.package.label2)}**`) : null}
          </div>
          <div>
