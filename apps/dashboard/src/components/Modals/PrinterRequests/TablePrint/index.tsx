...
export function TablePrinter({
  ...
  const { profile, getBartender, printStart, user } = useContext(AppContext);
  const cart = carts[0];
  ...
  transshipment = carts.reduce(
    (total, cart) => total + cart.transshipment(),
    0
  );
                ...
                <div className="mytest-2 underlineSeparator">
                  <p className="m-0 text-wrap">
                    ...
                    {prod.details.value > 0 ? (
                      <span className="m-0 ">
                        (
                        {`${currency({ value: prod.getTotal(true), withoutSymbol: true, currency: user?.controls?.currency })}`}
                        )
                      </span>
                    ) : null}
                  </p>
        ...
        {indexReq === 0 ? (
          <>
            ...
            {cart.type === "P" && (
              <tr>
                <td>
                  <span className="fw-bold">Data Entrega: </span>
            ...
            {props.type === "table" && (
              <tr>
                <td className="">
                  <span className="fw-bold">Permanência: </span>
  ...
  const content = (
    <table
      className={`table-printer mx-auto fs-${props.fontSize || 7} ${props.classBold || ""
        } ${props.className || ""} table-margin-${props.margin ?? 0}`}
      style={{
                                ...
                                {currency(
                                  {
                                    value: fee.type === "fixed" && fee.quantity
                                      ...
                                      (props.type === "command" ? command?.getTotalValue("command") || 0 :
                                        (opened ?? table?.opened)?.getTotalValue("table") ||
                                        0),
                                    withoutSymbol: true,
                                    currency: user?.controls?.currency
                                  }
      ...
      <tbody>
        ...
        {props.type === "table" && (
          <>
            ...
            <tr><td className="underlineSeparator"></td></tr>
          </>
        )}
      </tbody>
      <tbody id="print-foot">
        {carts[0]?.cupomId && carts[0]?.cupom && (
          <tr>
            <td className="pt-1">
              <div className="row-print clearfix">
                <div className="col-print-lt-payment">
                  <span className="fw-bold">Cupom Usado: </span>
                </div>
                <div className="col-print-rt-payment">
          ...
          carts[0].cupom?.type !== "freight" && (
            <tr>
              <td>
                <div className="row-print clearfix">
                  <div className="col-print-lt-payment">
                    <span className="fw-bold">Taxa Entrega: </span>
                  </div>
                  <div className="col-print-rt-payment">
                    <span>
        ...
        {carts[0]?.cupomId && (
          <tr>
            <td>
              <div className="row-print clearfix">
                <div className="col-print-lt-payment">
                  <span className="fw-bold">Cupom: </span>
                </div>
                <div className="col-print-rt-payment">
                  <span>
                    {carts[0].cupom?.type !== "freight"
                      ? `-${currency(
                        {
                          value: Number(carts[0].cupom?.type === "percent"
                            ? (subTotal / 100) *
          ...
          <>
            <tr>
              <td>
                <div className="row-print clearfix">
                  <div className="col-print-lt-payment">
                    <span className="fw-bold">Pago: </span>
                  </div>
                  <div className="col-print-rt-payment">
                    <span>{currency({
                      value: paid,
            ...
            <tr>
              <td>
                {(
                  props.type === "command" ? command : props.type === "table" ? table?.opened : cart
                )?.formsPayment.map((formPayment, index) => {
                  return <div key={'forms-payment-table-' + index} className="row-print clearfix">
                    <div className="col-print-lt-payment">
                      <p className="fw-boldtest m-0">{`\u2000\u2000Em ${formPayment.label}`}</p>
                      {formPayment.change ? <span>{`\u2000\u2000Troco`}</span> : null}
                    </div>
                    <div className="col-print-rt-payment">
                      <p className='m-0' >{currency({ value: formPayment.change ? formPayment.change : formPayment.value, withoutSymbol: true, currency: user?.controls?.currency })}</p>
                      {formPayment.change ? <span>{currency({ value: formPayment.change - formPayment.value, withoutSymbol: true, currency: user?.controls?.currency })}</span> : null}
                    </div>
                  </div>
                })}
              </td>
          ...
          (props.type === "command" || props.type === "table") && (
            <tr>
              <td>
                <div className="row-print clearfix">
                  <div className="col-print-lt-payment">
                    <span className="fw-bold">Faltam: </span>
                  </div>
                  <div className="col-print-rt-payment">
                    <span>{currency({ value: lack, withoutSymbol: true, currency: user?.controls?.currency })}</span>
                  </div>
                </div>
              </td>
            </tr>
