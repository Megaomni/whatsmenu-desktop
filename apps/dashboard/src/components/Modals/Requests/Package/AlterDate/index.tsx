  ...
  const setDate = async () => {
    if (daySelected) {
      try {
        ...
        setCarts([...carts])
        handleShowToast({ type: "success", title: "Alterar data" })
      } catch (error) {
        console.error(error)
        handleShowToast({ type: "erro", title: "Alterar data" })
      }
    }
  }
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            <h5>Alterar Data Pedido: wm{cart.code}-{cart.type}</h5>
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Row>
          ...
          <Row>
            <Col className="text-center">
              ...
              <p className={`mb-0 ${!daySelected && "invisible"}`}><span className="fw-bold">Data Escolhida : </span>
                {
                  daySelected && DateTime.fromJSDate(new Date(daySelected)).toFormat('dd/MM/yyyy')
                }
              </p>

              <p className="mt-0"><span className="fw-bold">Data Atual do Pedido : </span>
                {
                  DateTime.fromJSDate(new Date(cart.packageDate)).toFormat('dd/MM/yyyy')
                }
              </p>
            </Col>
          </Row>
