...
export function CropModal(props: CropModalTypes) {
  ...
  const { show, quality, onHide, aspectInitial,
    maxWidth, maxHeight, setImageBlob, typeCrop, inputFile } = props;
  ...
  async function createImage(crop: any, fileName: string, srcUrl?: string) {
    try {
      ...
      if (getMobileOS() === "iOS" && imageSrcCrop) {
        ...
        imageBlob = await picture.resize(imageElement, canvas, {
          unsharpAmount: 80,
          unsharpRadius: 0.6,
          unsharpThreshold: 2,
          filter: "box"
        ...
        >
          ...
          <Modal.Body id="modal-body" style={{ maxHeight: '70vh', overflow: 'auto' }}>
            <div className="d-flex justify-content-center gap-2">
              ...
              <div className="d-flex justify-content-center gap-2">
                <span>Largura: {widthCrop.toFixed(0)}px </span>
                <span>Altura: {heightCrop.toFixed(0)}px </span>
              </div>
            </div>
            <div style={{ display: 'flex', justifyContent: 'center' }}>
              <ReactCrop
                ...
                onComplete={(crop, percent) => {
                  setWidthCrop(crop.width); setHeightCrop(crop.height)
                }}
              />
            </div>
          </Modal.Body>
          <Modal.Footer>
            ...
            <Button
              onClick={async () => { await createImage(crop, `${profile.slug}${hash(15)}webp`); onHide() }} variant="success" autoFocus>Finalizar</Button>
          </Modal.Footer>
        </Modal>
      }
    </>
