...
export function Packages() {
  ...
  const $requests = sortedPackages.map((cart) => {
    const created_at = DateTime.fromSQL(cart.created_at).toFormat("dd/MM/yyyy");
      ...
      >
        ...
        >
          <div className="position-relative td-skip cursor-pointer">
            <BsPrinter size={25} color={`${cart.print ? "" : "red"}`} title={`${cart.print ? undefined : "Pedido não impresso"}`} />
          </div>
        </td>
        <td data-title="Cód. Pedido:">
        ...
        <td data-title="Data Entrega:">
          <span className="aling-middle fs-7">{packageDate}</span>
        </td>
        <td className="setPrint align-text-left td-skip" width={100} data-title="Entregador:">
          ...
          {cart.status !== "canceled" ? (
            <div className="d-flex gap-2 td-skip mx-auto " id="container-buttons" style={{ width: 275 }}>
              ...
              <SendStatusMessageForm
                ...
                newStatus="transport"
                button={{
                  name: !cart.address ? "Pronto Retirar" : "Entregando",
                  props: {
              ...
              <Card.Body>
                <Row className="fs-7 sm-package-component-header mb-3">
                  <Col className="d-flex align-items-center gap-2 wm-gap-row package-component-info">
                    <span className="text-nowrap">
                      Entregar
                      <span className="rounded-circle d-inline-block align-middle p-2 ms-2 wm-green-day"></span>{" "}
                      Hoje /
                      <span className="rounded-circle d-inline-block align-middle p-2 ms-2 wm-orange-day"></span>{" "}
                      Amanhã
                    </span>
                  </Col>
                ...
                {!cartsData.data.length && !profile.options.package.active ? (
                  <div className="p-5 my-5 text-center">
                    <h3>{textPackage(profile.options.package.label2)} Fechado</h3>
                    <span>Para receber pedidos de {textPackage(profile.options.package.label2)} habilite a opções nas configurações de {textPackage(profile.options.package.label2)}</span>
                  </div>
                ) : (
