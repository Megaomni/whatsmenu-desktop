...
export function TableComponent() {
  ...
  const handleEditTable = async () => {
    if (newName !== '' && newName !== table?.name) {
      ...
      return handleShowToast({
        type: 'alert',
        content: 'Por favor insira um nome válido ou diferente do nome atual',
        title: 'Alterar Mesa',
      })
    }
  }
  const handleDeleteTable = async () => {
    if (table) {
      try {
        ...
        handleShowToast({
          type: 'success',
          content: `Mesa ${table.name} deletada com sucesso`,
          title: 'Excluir Mesa',
        })
        setCurrentTableId(0)
      } catch (error: any) {
    ...
    if (table) {
      if (!table.opened?.commands.length || !table.status) {
        try {
          ...
          handleShowToast({
            type: 'success',
            content: `Mesa ${data.name} ${table.status ? 'ativada' : 'pausada'} com sucesso`,
            title: `Pausar Mesa`,
          })
        } catch (error) {
          console.error(error)
          handleShowToast({ type: 'erro', title: 'Pausar Mesa' })
        }
      } else {
        handleShowToast({
          type: 'alert',
          content: `Não é possivel pausar mesas com comandas abertas`,
          title: `Pausar Mesa`,
        })
      }
    }
          ...
          <Container fluid className="px-0 mx-0">
            <Row>
                  ...
                  <Row className="d-flex gap-2">
                    <Col className="text-nowrap d-flex flex-column align-items-center gap-2">
                      ...
                      <canvas id="qrcode" height={168} width={158} className="d-none">
                        <QRCodeCanvas
                          value={`${defaultDomain ?? 'whatsmenu.com.br'}/mesa/${table.id}/${profile.slug}`}
                          id="qrcodeCanvas"
                      ...
                      <div className="d-flex gap-2" style={{ width: '158px' }}>
                        ...
                        <Button
                          className="flex-grow-1"
                          onClick={() => handleCopy(`${defaultDomain ?? 'whatsmenu.com.br'}/mesa/${table?.id}/${profile.slug}`, handleShowToast)}
                        >
                          <FaShareAlt />
                        </Button>
                      </div>
              ...
              <Col sm>
                <h6>CONFIGURAÇÕES</h6>
                <Col sm="10">
                  <Form.Label className="mt-4">Nome da Mesa</Form.Label>
