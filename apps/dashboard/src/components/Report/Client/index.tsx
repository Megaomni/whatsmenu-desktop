...
export function ReportsLayout({ type, setData, setResume, ...props }: ReportsLayoutProps) {
  ...
  const handleGetReports = useCallback(async (page = 1, spinner = false) => {
    const body = { notValidate: true, filter: filter ?? 'delivery', columnDate: columnDate?.value ?? 'created_at', ...(type === "daily" ? { date } : { month, year }), payment };
    if (spinner) {
      setShowSpinner(true);
    }
    try {
        ...
        setData(prevData => {
          return {
            ...
            tables: (data.tables && {
              ...
              data: page === 1 ? data.tables?.data.map((t: Table) => new Table(t)) : [...prevData?.tables?.data, ...data.tables?.data.map((t: Table) => new Table(t))]
            }),
            carts: (data.carts && {
              ...data.carts,
              lastPage: data.carts.lastPage > 0 ? data.carts.lastPage : 1,
              data: page === 1 ? data.carts.data.map((c: Cart) => new Cart(c)) : [...prevData.carts.data, ...data.carts.data.map((c: Cart) => new Cart(c))]
            })
          }
        })
      }
                        ...
                        filter !== 'package' ?
                          ...
                          <InputGroup>
                            ...
                            <DropdownButton title={columnDate?.name ?? 'Buscar por'}>
                              {
                                [{ name: 'Data Criação', value: 'created_at' }, { name: 'Data Entrega', value: 'packageDate' }].map(item => {
                                  return <Dropdown.Item key={item.name} onClick={() => setColumnDate(old => item as any)}>
                                    {item.name}
                                  </Dropdown.Item>
                                })
                              }
                  ...
                  <>
                    ...
                    </Col>
                    {
                      ...
                      <Col sm>
                        ...
                        <Form.Select
                          onChange={(e) => setColumnDate(old => JSON.parse(e.target.value))}
                        >
                          {[{ name: 'Data Criação', value: 'created_at' }, { name: 'Data Entrega', value: 'packageDate' }].map((item) => (
          ...
          <div className="flex-column flex-lg-row d-flex justify-content-between gap-4" >
            <div className="d-flex flex-column justify-content-between flex-grow-1" >
              <div className="border p-2">
                <h6>Resumo:</h6>
                <p className="mb-1">Pedidos: {currency({ value: props.resume?.totalCarts ?? 0, currency: user?.controls?.currency })} {filter === "table" && <span className="ms-1">(Sem Taxas)</span>}</p>
                ...
                <div className="d-flex align-items-center mb-1">
                  <p className="m-0">
                    Total: {currency({ value: props.resume ? props.resume.total - (Object.values(props.resume.cuponsResume)).reduce((total, cupom) => total += cupom.value, 0) : 0, currency: user?.controls?.currency })} {filter === "table" && <span className="ms-1">(Sem Taxas)</span>}
                  </p>
                  {filter !== "table" ? (
                    <OverlayTrigger placement="bottom" overlay={
                      <Popover id="popover-basic" style={{ minWidth: '330px' }}>
                        <Popover.Body>
                          Total de vendas do período - Total de cupons
                        </Popover.Body>
                      </Popover>
                    }>
                      <Button variant="none" className="px-2"><BsInfoCircle size={18} /></Button>
                    </OverlayTrigger>
                  ) : null}
              ...
              {filter === 'table' ? <div>
                <div className="border p-2">
                  <h6>Taxas:</h6>
                  {feesResume && Object.keys(feesResume).length ? Object.entries(feesResume).map(([code, value]) => (
                    <p key={code} className="mb-1" >{code}: {currency({ value, currency: user?.controls?.currency })}</p>
                      ...
                      <tr>
                        <td className="position-sticky bg-white p-0" style={{ top: 0 }} colSpan={3}>
                          <div className="d-flex justify-content-between  p-2">
                            <div>Cupons:</div>
                            <div>Total: {currency({ value: (Object.values(props.resume.cuponsResume)).reduce((total, cupom) => total += cupom.value, 0), currency: user?.controls?.currency })}</div>
                          </div>
                        </td>
                      </tr>
                    </thead>
                          ...
                          return (
                            <tr key={cupom.code}>
                              <td className={`text-wrap`}>
                                {cupom.deleted_at ? cupom.code.replace(
                                  cupom.code.substring(cupom.code.length - 20),
                                  " (Desativado)") : cupom.code}
                              </td>
                              <td>{cupom.type}</td>
                              <td className={`text-wrap`}>
                                {currency({ value: cupom.value, currency: user?.controls?.currency })}
                              </td>
                            </tr>
                          )
                        }
              ...
              <>
                    ...
                    <tbody>
                      {Object.entries(props.resume.formsPaymentResume).map((formPayment: [string, any]) => (
                        <tr key={formPayment[0]} className={formPayment[0] === payment ? 'bg-primary' : ''}>
                          ...
                          <td className={formPayment[0] === payment ? 'text-white text-end' : ' text-end'}>{currency({ value: formPayment[1], currency: user?.controls?.currency })}</td>
                        </tr>
                      ))}
                    </tbody>
                    <tfoot>
                ...
                <div className="flex-grow-1 overflow-auto" style={{ maxHeight: '410px' }}>
                  <TableBs bordered striped className="m-0">
                    ...
                    <tbody>
                      {bartenders.sort((a, b) => !a.deleted_at ? -1 : 1).map((bartender) => instanceActivedDate(bartender) && ( // props.resume?.bartendersResume[bartender.id] > 0 &&
                        <tr key={bartender.id}> {/* className={bartender.id === bartenderId ? 'bg-primary' : ''}*/}
                          <td className={`text-wrap`}> {/*  ${bartender.id === bartenderId ? 'text-white' : ''} */}
                            {bartender.deleted_at ? bartender.name.replace(
                              bartender.name.substring(bartender.name.length - 19),
                              " (Desativado)") : bartender.name}
                          </td>
                          <td>{currency({ value: props.resume?.bartendersResume[bartender.id], currency: user?.controls?.currency })}</td> {/* className={bartender.id === bartenderId ? 'text-white text-end' : ' text-end'} */}
                        </tr>
                      ))}
                    </tbody>
                  </TableBs>
