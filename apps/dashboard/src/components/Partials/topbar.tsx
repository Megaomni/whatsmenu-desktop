            ...
            <>
              ...
              >
                <span className="with-icon">
                  <BsEye />
                  {window.innerWidth <= 426 ? null : <span>{t('view_menu')}</span>}
                </span>
              </Button>
              <Button
                as="a"
              ...
              {plansCategory.includes('table') ? (
                <Button
                  ...
                  onClick={() => {
                    if (bartenders.filter((bartender) => !bartender.deleted_at).length) {
                      window.open(`${process.env.WHATSMENU_BASE_URL}/${profile.slug}/${t('tables')}`, '_blank')
                    } else {
                      handleConfirmModal({
                ...
                >
                  <GiRoundTable />
                  {window.innerWidth <= 426 ? null : <span>{t('waiter')}</span>}
                </Button>
              ) : null}
              ...
              <Dropdown autoClose="outside" onToggle={(e) => !e && setRequestsCount(0)} className="nav-link nav-icon position-relative p-0">
                ...
                <Dropdown.Toggle variant="outline-success" className={`h-100 px-2 fw-normal m-0 ${window.innerWidth <= 426 && 'gap-0'}`} size="sm">
                  <>
                    <ImTicket />
                    {window.innerWidth <= 426 ? null : <span>{t('orders')}</span>}
                  </>
                </Dropdown.Toggle>
