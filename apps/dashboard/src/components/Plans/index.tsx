...
export function Plans({
  ...
  const getPlanProperty = useCallback((plan: Plan, currency: string = "brl", period: 'monthly' | 'semester' | 'yearly' = 'monthly') => {
  ...
  const $plans = (type: 'register' | 'upgrade') => {
    if (principalPlan?.relateds?.length || plans?.length) {
      return (type === "register" ? plans : principalPlan?.relateds)?.filter(plan => plan.category !== principalPlan?.category).map(plan => {
        if (plan.type === type) {
          ...
          return (
            <Form.Check
              ...
              label={`${plan.name} - ${currency({ value: Number(planValue), currency: user.controls?.currency })} ${!plan.status ? "(Plano Antigo)" : userDeletePlans.some(dPlan => dPlan.id === plan.id) ? `(Será removido)` : ""}`}
              onChange={(e) => {
  ...
  const avaliablePlans = useCallback((reavaliable?: boolean) => {
  ...
  useEffect(() => {
    setCart && setCart(old => {
      ...
      if (principalPlan) {
        const plansToCalcValue = [principalPlan, ...userOtherPlans];
        const plansItems = plansToCalcValue.map(plan => {
        ...
        <Card.Body className="wm-default text-dark text-start">
                  ...
                  if (price_id) {
                    return (
                      <option
                        ...
                        value={plan.id}
                      >
                        {plan.name} - {currency({ value: Number(planValue), currency: user?.controls?.currency })}
                      </option>
                    )
                  }
                })}
          ...
          <>
            ...
            <div className="d-flex gap-0 gap-md-4 fw-bold flex-column flex-md-row">
              <span>Total: {currency({ value: total, currency: user?.controls?.currency })}</span>
