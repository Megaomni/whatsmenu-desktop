...
export function CardDetails({
  ...
  const { handleShowToast, gateway, user, dispatchUser } =
    useContext(AppContext)
  const [cards, setCards] = useState<Card[]>([])
  const [default_card, setDefaultCard] = useState<string | undefined>(
    user?.controls?.paymentInfo?.default_card
  )
  ...
  const createOrEditCard = async (e: FormEvent) => {
    ...
    const inputNumber = form.querySelector(
      'input[name=number]'
    ) as HTMLInputElement
    const inputName = form.querySelector(
      'input[name=holderName]'
    ) as HTMLInputElement
    const inputCvv = form.querySelector('input[name=cvv]') as HTMLInputElement
    const inputMonth = form.querySelector(
      'input[name=expMonth]'
    ) as HTMLInputElement
    const inputYear = form.querySelector(
      'input[name=expYear]'
    ) as HTMLInputElement
    ...
    switch (user?.controls?.paymentInfo?.gateway) {
      case 'pagarme':
        try {
          ...
          const {
            data: { card: newCard },
          } = await gateway?.createCard(token.id)
          const userCards = user.controls.paymentInfo.cards
            ? user.controls.paymentInfo.cards.filter(
          ...
          handleShowToast({
            title: 'Cartão',
            content:
              'Houve um problema ao adicionar seu cartão, verifique os dados e tente novamente',
            type: 'erro',
          })
        }
      ...
      case 'stripe':
        ...
        const newCards = [
          ...cards.filter(
            (item) => !(dataNewCard && dataNewCardId === item.id)
          ),
          ...[newCard],
        ]
  ...
  const deleteCard = async (cardId?: string, newCard?: boolean) => {
    try {
      if (cardId) {
        ...
        const newCards = user.controls.paymentInfo.cards.filter(
          (card) => card.id !== cardId
        )
      ...
      handleShowToast({
        title: 'Excluir Cartão',
        content:
          'Cartão não foi excluído, é necessário cancelar as inscrições vinculadas a esse cartão neste painel',
      })
    }
  }
  ...
  useEffect(() => {
    if (
      user?.controls?.paymentInfo?.cards &&
      user.controls.paymentInfo.cards.length
    ) {
      const userCards = user?.controls?.paymentInfo.cards
  ...
  const popoverHelpCVV = (
    <Popover title="Código de Segurança">
      <div className="p-1">
        <h4 className="fs-6 mb-1 mt-1 p-0 text-center">Código de Segurança</h4>
        <img
          src="/images/cartao.webp"
          alt="Help Card"
          width="200px"
          height="130px"
        />
  ...
  return (
    <Container className="d-flex flex-column bg-gray h-100 position-relative p-1">
      <Row>
        <Col>
          <h5>{review ? 'Revise os ' : ''}Dados para Pagamento</h5>
          <span className="fs-7">
            As informações, não ficarão salvas em nossa base de dados.
          </span>
        </Col>
                  ...
                  <div>
                    <Row>
                      <Col>
                        <div className="d-flex align-items-center gap-2">
                          <h5 className="m-0 p-0">
                            {index + 1} - Cartão{' '}
                            {card.brand ? card.brand : 'Novo'}{' '}
                            {card.status === 'expired' ? '(EXPIRADO)' : null}{' '}
                          </h5>
                        ...
                        {!card.new && (
                          <>
                            {card.id !== default_card && (
                                  ...
                                  if (card.id) {
                                    try {
                                      ...
                                      if (
                                        ...
                                        await gateway?.changeSubscriptionCard(
                                          card.id
                                        )
                                      }
                                      ...
                                      handleShowToast({
                                        title: 'Alterar Cartão',
                                        content:
                                          'Não foi possível alterar seu cartão tente novamente',
                                      })
                                    }
                              ...
                              >
                                Usar este cartão para cobrança
                              </Button>
                            )}
                          </>
                        )}
                      ...
                      {default_card !== card.id && (
                        <Col className="d-flex justify-content-end">
                          <Button
                            variant="link"
                            onClick={() => deleteCard(card.id, card.new)}
                          >
                            Excluir
                          </Button>
                        </Col>
                    ...
                    <Row className="justify-content-center ">
                      <Col>
                        <Form
                          method="POST"
                          onSubmit={createOrEditCard}
                          id={`card_data_${card.id}`}
                          data-new-card={!!card.new}
                          data-new-card-id={card.id}
                        >
                          <Row>
                            <Col>
                              ...
                              <Form.Control
                                ...
                                name="number"
                                defaultValue={
                                  !card.new ? card.number : undefined
                                }
                                minLength={19}
                                maxLength={19}
                                disabled={!card.new}
                                ...
                                onChange={(e) => {
                                  ...
                                  e.target.value = numbers.replace(
                                    /(\d{4})(?=\d)/g,
                                    '$1 '
                                  )
                                }}
              ...
              <Button
                onClick={async () => {
                  ...
                  handleShowToast({
                    title: 'Dados de pagamento',
                    content:
                      'É necessário adicionar um cartão antes de continuar',
                  })
                }}
              >
      ...
      <Modal
        ...
        centered
      >
        <Modal.Header>Escolha a quantidade de parcela</Modal.Header>
        <Modal.Body>
          <Row className="mb-3">
            <Col className="d-flex gap-3">
              ...
              >
                {Array(installments)
                  ...
                  .map((item, index) => {
                    return (
                      <option key={item + (index + 1)} value={index + 1}>
                        Parcelar em {index + 1}x
                      </option>
                    )
                  })}
              </Form.Select>
                ...
                onClick={async () => {
                  if (default_card) {
                    try {
                      ...
                      await onClickContinue(
                        default_card,
                        installmentsModal.installments,
                        installmentsModal.type
                      ).catch(() => setLoading(false))
                      ...
                      handleShowToast({
                        title: 'Aviso',
                        content:
                          'Desculpe, houve uma falha no processamento, recarregaremos a página',
                      })
                      console.error(error)
                    }
                  }
