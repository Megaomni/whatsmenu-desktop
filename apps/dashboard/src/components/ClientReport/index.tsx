    ...
    <>
      <Card>
        <Card.Header className="d-flex gap-3">
          ...
          <HelpVideos.Trigger urls={[{ src: 'https://www.youtube.com/embed/hMTYKIVi9nU', title: 'Relatório de Clientes' }]} />
        </Card.Header>
        <Card.Body>
          <SearchForm
                ...
                <section ref={ref} id="printReport">
                  <Container fluid className="mx-0 p-0">
                    <Row className="notPrint px-3 my-2 align-items-center">
                      <Card as="form" onSubmit={handleSubmit(fetchTop10ByDate)} onReset={() => {
                        reset()
                        fetchTop10ByDate({})
                      }}>
                        <Card.Body className='d-flex flex-column flex-md-row gap-2'>
                          ...
                          <Form.Label className='m-0' >
                            <span>Data de final</span>
                            <Form.Control type="date" {...register('endDate', { required: false })} min={DateTime.fromFormat(watch('startDate') || DateTime.local().toFormat('yyyy-MM-dd'), 'yyyy-MM-dd').toFormat('yyyy-MM-dd')} max={DateTime.local().toFormat('yyyy-MM-dd')} />
                          </Form.Label>
                          <Button className='mt-auto' type="submit" variant="success">Filtrar</Button>
                          <Button className='mt-auto' type="reset" >Limpar</Button>
                        </Card.Body>
                      </Card>
                    </Row>
