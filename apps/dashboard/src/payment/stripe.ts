...
export default class StrategyStripe implements GatewayStrategy {
    ...
    public async addSubscriptionItem(item: AddSubscriptionItemContracts) {
        return await apiRoute("/stripe/addSubscriptionItem", this.session, "POST", item);
    };

    async chargeAutomatically(data: ChargeAutomaticallyContracts) {
        return await apiRoute("/stripe/createCheckoutOrPurchaseData", this.session, "POST", data);
    }

    public async changeChargeCard(cardId: string) {
        return await apiRoute("/stripe/changeChargeCard", this.session, "PATCH", { cardId });
    }

    public async changeSubscriptionCard(cardId: string) {
        return await apiRoute("/stripe/changeSubscriptionCard", this.session, "PATCH", { cardId });
    }
