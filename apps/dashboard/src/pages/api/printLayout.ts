...
import {
  NotePrint,
  NotePrintProps,
} from '@components/Modals/PrinterRequests/NotePrint'
...
export default async function PrintLayout(
  ...
  if (req.method === 'POST') {
    try {
      ...
      const props: Partial<NotePrintProps> = {
        cart: new Cart(cart),
        profile: profile,
      ...
      const layout58 = renderToString(
        createElement(NotePrint, { ...props, paperSize: 58 } as any)
      )
      const layout80 = renderToString(
        createElement(NotePrint, { ...props, paperSize: 80 } as any)
      )
