            ...
            <Form onSubmit={handleSubmit}>
              <Row>
                ...
                <Col sm>
                  ...
                  >
                    {Array.from(Array(12).keys()).filter(m => {
                      m = Number(m + 1)
                      ...
                      return (
                        <option key={month} value={monthValue}>{monthValue}</option>
                      )
                    })}
                  </Form.Select>
                </Col>
            ...
            <Card className="mt-3">
              ...
              <Card.Body>
                <Table responsive striped hover>
                  ...
                  <tbody>
                    {leads?.map((lead, index) => ({ ...lead, id: index })).reverse().map((lead) => (
                      <tr key={`${lead.id}`} className="fs-7 text-center">
                        ...
                        <td style={{ maxWidth: '200px', overflow: 'hidden' }}>{DateTime.fromISO(lead.date).toFormat('dd/MM/yyyy HH:mm:ss')}</td>
                        <td style={{ maxWidth: '200px', overflow: 'hidden' }}>{lead?.seller?.name ?? '-'}</td>
                        <td>
                          <Button id={`lead-${lead.id}`} className="w-100" variant={lead.contacted_at ? 'outline-success' : 'primary'} onClick={() => handleUpdateLead(lead.id)} disabled={!!lead.contacted_at === true}>
                            {lead.loading ? (
                              <>
                              ...
                              <>
                                {lead.contacted_at ? (
                                  <>
                                    <BsCheck />
                                    <span>Contatado em {DateTime.fromISO(lead.contacted_at).toFormat('dd/MM/yyyy HH:mm')}</span>
                                  </>
                                ) : `Contatado`}
                              </>
                            )}
