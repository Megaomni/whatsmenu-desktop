...
import { apiRoute, copy, currency, hash, inputFocus, mask } from "../../../utils/wm-functions";


export default function Products({ plans, products }: { plans: Plan[], products: SystemProduct[] }) {
  const { data: session } = useSession();

  const { handleShowToast, handleConfirmModal } = useContext(AppContext);
    ...
    try {
      if (modalConfig.product && modalConfig.product.name.length) {
        ...
        if (modalConfig.product && modalConfig.type !== 'none') {
          ...
          const { data: { systemProduct: data } } = await apiRoute(`/adm/system/products`, session, modalConfig.type === "create" ? 'POST' : 'PATCH', {
            gateway,
          ...
          handleShowToast({
            ...
            content: `${modalConfig.category === 'plans' ? 'Planos' : 'Produtos'} ${modalConfig.type === 'create' ? 'criados' : 'atualizados'} com sucesso.`,
            type: 'success'
          });
      ...
      handleShowToast({
        show: true,
        title: modalConfig.category === 'plans' ? 'Planos' : 'Produtos',
        content: `${modalConfig.category === 'plans' ? 'Planos' : 'Produtos'} não foram ${modalConfig.type === 'create' ? 'criados' : 'atualizados'}.`,
        type: 'erro'
      })
    }
  }
  ...
  return <>
                ...
                return (
                  <Accordion key={plan.name + plan.id} className="border mb-1">
                    <Accordion.Header className="fs-5">
                      <Row className="w-100">
                        <Col md={2}><span>{plan.id} - {plan.name}</span></Col>
                        <Col md={2}><span>{plan.type === 'register' ? 'Normal' : 'Desconto'}</span></Col>
                        <Col md={2}><span>{plan.category === 'basic' ? 'Básico' : plan.category === 'table' ? 'Mesas' : 'Encomendas'}</span></Col>
                        <Col md={2}><span style={{ color: plan.status ? '' : 'red' }}>{plan.status ? 'Ativo' : 'Inativo'}</span></Col>
                      </Row>
                    </Accordion.Header>
                    <Accordion.Body>
                      <Row>
                        {[
                          ...
                          if (planProduct) {
                            return <Col key={item.period} sm={12 / arr.length} className={`d-flex flex-column ${index > 0 ? "border-start" : ""}`}>
                              <h5>{item.name}</h5>
                              <div className="">
                                <div className="ps-2">
                                  {
                                    ...
                                    <Row>
                                          ...
                                          return <Col sm={12} key={price.id}>
                                            ...
                                            </Row>
                                            {
                                              Object.entries(price.currencies).map((currencyMoney, index) => {
                                                return <Row key={currencyMoney[0] + index}>
                                                  <Col>
                                                    <p className="m-0 fs-7">Moeda <span className="fw-bold">{currencyMoney[0].toUpperCase()}</span> - {currency({ value: currencyMoney[1].unit_amount / 100, currency: currencyMoney[0] })}</p>
                                                  </Col>
                                                </Row>
                                              })
                                            }
                                      ...
                                      <Col sm="12" className="pt-4">
                                        <div className="mt-auto  justify-content-between  d-flex gap-2">
                                          <Button size="sm"
                                            variant="danger"
                                            onClick={() => deleteProductOrPlanStripe(planProduct.id, item.period)}>
                                            Excluir
                                          </Button>
                            ...
                            return <Col key={item.period} className={`ms-2 ${index > 0 ? "border-start" : ""}`}>
                              <h5>{item.name}</h5>
    ...
    <Row>
      <Col className="p-0">
        <Card>
          ...
          <Card.Body>
            {
              products.filter(prod => prod.service !== 'plan').map(systemProduct => {

                return <Accordion key={systemProduct.default_price} className="border mb-1">
                  <Accordion.Header>
                    ...
                    <Row>
...

                      {
                        systemProduct.operations.prices.map((price, index, arrPrice) => {
                          return <Col sm={3} className="fs-6" key={`${price.id}`}>
                            <Row>
                              <Col sm={12} key={price.id} className={`ps - 4 mb - 2 ${index > 0 ? "border-start" : ""} `}>
                                                                      ...
                                                                      </div> */}
                                {
                                  Object.entries(price.currencies).map(currencyType => {
                                    ...
                                    return <div key={`${price.id}-${currencyName}`}>
                                      <p className="m-0 fs-7">Moeda <span className="fw-bold">{currencyName.toUpperCase()}</span>: {currency({ value: currency_amout / 100, currency: currencyName })}</p>
                                    </div>
                                  })
                                }
                              </Col>
                      ...
                      <Col className="d-flex gap-2 align-items-center justify-content-end mb-2">
                        <div className="d-flex gap-3">
                          <Button
                            size="sm"
                            variant="danger"
                            onClick={() => deleteProductOrPlanStripe(systemProduct.id)}>Excluir</Button>
                          <Button
                            size="sm"
                            onClick={(e) => {
                              e.stopPropagation();
    ...
    >
      <Modal.Header closeButton>
        {modalConfig.type === "create" ? "Criando" : modalConfig.type === "update" ? "Atualizando" : ""} Produto {modalConfig.plan?.name ? modalConfig.plan.name : ""} <span className="fw-bold ms-1"> {modalConfig.category === 'plans' ? `- Plano ${getRecurringType(modalConfig.recurring?.interval ?? '')} ` : ''}</span>
      </Modal.Header>
      <Modal.Body className="overflow-auto modal-system-products">
        <Row>
          ...
          <Col sm="12">
            {
              modalConfig.product?.operations.gateways && Object.entries(modalConfig.product.operations.gateways).map((gat, index, arr) => {
                const name = gat[0].split("").map((lr, index) => index === 0 ? lr.toUpperCase() : lr).join("");
          ...
          <Col sm="12">
            <Form.Label>Nome<sup style={{ color: 'red' }}>*</sup></Form.Label>
            <FormControl
        ...
        </Row>
        {
          ...
          <Row className="mt-2">
            <Col>
              ...
              <Form.Select defaultValue={modalConfig.product?.service} disabled={modalConfig.type === 'update'}
                onChange={(e) => {
                  if (modalConfig.product) {
                    setModalConfig({
                      ...modalConfig,
              ...
              >
                <option disabled {...(modalConfig.product?.service ? {} : { selected: true })}>Selecione uma opção</option>
                {
                  [{
                    value: 'printer',
                    name: 'Impressora'
                    ...
                    <div >
                      <Row className="my-1 align-items-center mb-2">
                        ...
                        <Col sm={5} className="d-flex justify-content-end gap-2 align-items-center">
                          <label className="text-nowrap" title="Moeda padrão do preço, é a moeda que será cobrada se nenhuma for encontrada.">Padrão :</label>
                          <Form.Select disabled={modalConfig.type === 'update' && !price.new} defaultValue={price.default_currency} onChange={(e) => {
                            if (modalConfig.product) {
                          ...
                          }}>
                            {
                              Object.entries(price.currencies).map((currency, index) => {
                                return <option key={currency[0] + index} value={currency[0]}>{currency[0].toUpperCase()}</option>
                              })
                            }
                          </Form.Select>
                        </Col>
                        ...
                        ['brl', 'eur', 'usd'].map((currencyType, index, arrCurrency) => {
                          ...
                          return (
                            <Row className="mb-2 align-items-center" key={currencyType[0] + index}>
                              <Col sm={3}>
                                <p className="m-0" style={{ whiteSpace: "nowrap" }}>Moeda <span className="fw-bold">{currencyType.toUpperCase()}</span>: </p>
                              </Col>
                              <Col>
                                <div>
                                  <InputGroup>
                                    <InputGroup.Text style={{ width: 60 }}>{currency({ value: 0, currency: currencyType, symbol: true })}</InputGroup.Text>
                                    <FormControl
                                      id={`${currencyType}-${price.id}`}
                                      className={modalConfig.type === 'update' && !price.new ? 'not-allowed' : ''} disabled={modalConfig.type === 'update' && !price.new}
                                      defaultValue={currency({ value: price.currencies[currencyType]?.unit_amount / 100, currency: currencyType, withoutSymbol: true })}
                                      onChange={(e) => {
                                        mask(e, "currency");
      ...
      <Modal.Footer className="justify-content-between">
        <Button variant="danger" disabled={!modalConfig.product?.operations.prices.length} onClick={() => setModalConfig({ category: "none", type: "none", show: false })}>Cancelar</Button>
        <Button disabled={!modalConfig.product?.operations.prices.length} onClick={createOrUpdate}>Finalizar</Button>
      </Modal.Footer>
    </Modal>
  </>;
}
