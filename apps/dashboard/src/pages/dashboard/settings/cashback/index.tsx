...
const CashbackFormSchema = z.object({
  percentage: z.string().transform((value) => parseInt(value)).refine((value) => value > 0 && value <= 100, { message: 'Porcentagem inválida' }),
...
export default function SettingsCashback() {
  ...
  const { register, handleSubmit, formState: { errors } } = useForm<CashbackFormInput>({
    ...
    <>
      ...
      <Card>
        <Card.Header className="d-flex justify-content-end align-items-center">
          <Form.Switch id="toggle-cashback" label="Ativar Cashback" checked={profile.options.voucher[0].status} onChange={handleToggleCashbackStatus} />
        </Card.Header>
        <Card.Body>
          <form id="form-cashback" onSubmit={handleSubmit(handleSubmitCashback)} className="d-flex flex-column flex-md-row gap-3">
            <div>
              ...
              <InputGroup className="position-relative">
                <Form.Control isInvalid={!!errors.percentage} type="number" placeholder="Porcentagem. Ex: 10" {...register("percentage", { max: 100, min: 1 })} />
        ...
        <Card.Footer className="d-flex justify-content-end">
          <Button variant="success" type="submit" form="form-cashback">Salvar</Button>
        </Card.Footer>
      </Card>
    </>
  )
