...
export default function Profile({ }: ProfileProps) {
  ...
  return (
    profile &&
    <>
      <Title title="Meu Perfil" componentTitle="Perfil" className="mb-4" />
      {steps && (
        <div className="bd-callout bd-callout-success">
          <p className="m-0">
            <span className="fw-bold">Parabéns</span>, {(session?.user?.name ?? session?.user?.$attributes.name).split(" ")[0]}! <br />
            Em poucos passos você terá um atendimento ágil e organizado com o seu
            <span className="fw-bold"> Cardápio Digital.</span>
          </p>
          <div className="d-flex align-itens-center justify-content-between">
            <p className="m-0 my-3 text-red-400">
              Preencha agora os dados básicos do seu estabelecimento.
            </p>
            <HelpVideos.Trigger urls={[{ src: "https://www.youtube.com/embed/a7kfhg0lKFM", title: "Preenchendo perfil" }]} />
          </div>
        </div>
      )}
      <section>
                  ...
                  </Tab.Pane>
                  {
                    <>
                      ...
                      <Tab.Pane eventKey="forms-payment">
                        <PaymentMethodProvider>
                          {profile.id && <ProfileFormsPayment isActive={key === 'forms-payment'} />}
                        </PaymentMethodProvider>
                      </Tab.Pane>
                    </>
                  }
