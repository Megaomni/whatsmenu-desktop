...
class PaymentController {
  async generateRequest({ response }) {
    try {
      ...
      let users = await User.query()
        ...
        .whereRaw(`controls->"$.canceled" is null or (due = ${moment().add(7, 'days').format('DD')} && controls->"$.canceled" = false)`)
      ...
      if (moment().add(7, 'days').format('MM-DD') === '02-28') {
        ...
        users = await User.query()
          .where('due', '>=', 28)
          .whereRaw('controls->"$.canceled" is null or controls->"$.canceled" = false')
          .whereRaw('controls->"$.type" is null')
          .whereRaw('controls->"$.migrated_at" is null')
          .whereRaw('controls->"$.paymentInfo" is null')
          .with('invoices', (invoice) => {
      ...
      if (moment().add(7, 'days').format('DD') === '30' && moment().add(8, 'days').format('DD') === '01') {
      ...
      for (const user of users.rows) {
        ...
        const { value, items } = PaymentController.getDataItemsAndValue(plans, userPlans, products, user)
        ...
        let invoice = await user
          ...
          .where((query) => {
            if (userParser.invoices.length > 0) {
              return query.where('expiration', 'like', `${moment().add(7, 'days').format('YYYY-MM-')}%`)
            }
        ...
        if (!invoice || (invoice.status === 'paid' && !moment().format('YYYY-MM') === moment().add(7, 'days').format('YYYY-MM'))) {
          ...
          if (invoice.status === 'paid') {
            const checkBonusSupport = await Invoice.query().where({ userId: user.id, type: 'monthly' }).limit(3).fetch()
            if (checkBonusSupport.rows.length === 1 && moment().diff(moment(user.created_at), 'days') < 40) {
              BonusSupport.create({
                userId: user.id,
        ...
        if (
          ...
          (!user.controls.period || user.controls.period == 'monthly' || user.controls.nextInvoice == moment().add(7, 'days').format('YYYY-MM-DD'))
        ) {
          ...
          if (invoice.type === 'first') {
            const requests = await invoice.requests().where('status', 'pending').whereNull('limit').fetch()
            ...
            if (requests.rows.length === 0) {
              ...
              const data = {
                ...
                payer_cpf_cnpj: user.secretNumber.split('.').join('').replace('/', '').replace('-', ''),
                type_bank_slip: 'boletoA4',
                days_due_date: 7,
                payer_phone: user.whatsapp.replace(' ', '').replace('-', ''),
                notification_url: Env.get(user.controls.migrated_at ? 'PAGHIPER_RETURN_NEXT' : 'PAGHIPER_RETURN'),
                // items: []
                items: plans.rows.map((plan) => {
                  const item = items.find((item) => item.category === plan.category)

                  return {
                    item_id: plan.id,
                    description: `${plan.name}`,
                    seller_description: `Referente a utilização do sistema WhatsMenu de ${moment(request.expiration).format(
                      'DD-MM-YYYY'
                    )} até ${moment(request.expiration).add(29, 'days').format('DD-MM-YYYY')}`,
                    quantity: 1,
              ...
              const paghiper = await axios.post('https://api.paghiper.com/transaction/create/', data)
      ...
      Mail.send('email.reports.generateinvoices', { invoices: itens }, (message) => {
        message
          .to('jasonaries@gmail.com')
          .from('jasonaries@whatsmenu.com.br')
          .subject(`Geração de mensalidades ${moment().format('YYYY-MM-DD HH:mm:ss')}`)
      })
    ...
    try {
      ...
      for (const user of users.rows) {
        ...
        let invoice = await user
          ...
          .where((query) => {
            if (userParser.invoices.length > 0) {
              return query.where('expiration', 'like', `${moment().add(req.days, 'days').format('YYYY-MM-')}%`)
            }
        ...
        if (!invoice || (invoice.status === 'paid' && !moment().format('YYYY-MM') === moment().add(req.days, 'days').format('YYYY-MM'))) {
          invoice = await Invoice.create({
        ...
        if (
          ...
          (!user.controls.period ||
            ...
            user.controls.nextInvoice == moment().add(req.days, 'days').format('YYYY-MM-DD'))
        ) {
          // const requests = await user.requests().where('type', 'M').where('expiration', 'like', `${moment().add(7, 'days').format('YYYY-MM-')}%`).whereIn('status', ['paid', 'pending', 'reserved', 'completed']).last()
  ...
  async returnPaghiper({ request, response }) {
    try {
      ...
      const addBonusSupport = async (user, systemRequest) => {
        try {
          const invoice = await user.invoices().where({ type: 'monthly', status: 'paid' }).first()
          const today = moment()

          if (invoice && invoice.id === systemRequest.invoiceId && today.diff(invoice.updated_at, 'days') < 24) {
            const exists = await BonusSupport.query().where('invoiceId', invoice.id).first()

            if (exists) {
              exists.status = systemRequest.status === 'canceled' ? 'canceled' : 'paidLate'
              await exists.save()
            } else {
      ...
      async function cancelOthersInvoices() {
        try {
          const invoices = await user.requests().where('status', 'pending').fetch()
      ...
      if (!user.controls.disableInvoice && systemRequest && systemRequest.status !== 'paid' && systemRequest.status !== 'completed') {
          ...
          switch (data.status.toLowerCase()) {
              ...
              if (invoice) {
                ...
                if (invoice.type === 'first') {
                  if (user.controls.period == 'semester') {
                    user.controls.nextInvoice = moment().add(6, 'months').format()
                    await user.save()
                  } else if (user.controls.period == 'yearly') {
                    user.controls.nextInvoice = moment().add(1, 'years').format()
                    await user.save()
                  }
            ...
            case 'reserved':
              ...
              if (invoice) {
                ...
                if (invoice.type === 'first') {
                  if (user.controls.period == 'semester') {
                    user.controls.nexInvoice = moment().add(6, 'months').format()
                  } else if (user.controls.period == 'yearly') {
                    user.controls.nexInvoice = moment().add(1, 'years').format()
                  }
                } else if (invoice.type === 'upgrade') {
                  await UserPlan.create({
                    userId: user.id,
        ...
        if (systemRequest.status === 'reserved' || systemRequest.status === 'paid' || systemRequest.status === 'completed') {
      ...
      if (Env.get('NODE_ENV') === 'production') {
        ...
        const notification = await axios.post('https://api.paghiper.com/transaction/notification/', data)
    ...
    try {
      ...
      if (!user.controls.disableInvoice) {
        ...
        const data = {
          ...
          type_bank_slip: 'boletoA4',
          days_due_date: 1,
          seller_description: `Referente a utilização do sistema WhatsMenu de ${moment(request.expiration).format('DD-MM-YYYY')} até ${moment(
            request.expiration
          )
            .add(29, 'days')
            .format('DD-MM-YYYY')}`,
          ...
          items: invoice.itens.map((item) => {
            if (typeof item === 'string') {
              ...
              const value = item.service ? item.value * item.quantity - discount : item.value
              return {
                item_id: item.id,
  ...
  async createFirstRequest(userId, cartItems, installments = 1) {
    try {
      ...
      invoice.status = user.controls.disableInvoice && !user.controls.paymentInfo ? 'paid' : 'pending'
      ...
      const invoiceValue = cartItems.reduce((a, item) => item.value * item.quantity - item.value * (item.quantityDiscount || 0) + a, 0)
  ...
  static async createNewInvoiceToUser(userId, value = null) {
    try {
      ...
      if (!user.controls.disableInvoice) {
        ...
        const monthRange = moment().diff(moment().subtract(1, 'months'), 'days')
        const newInvoice = {
          expiration: invoice.expiration,
          ...
          items: [
            {
              ...
              quantity: 1,
              price_cents: value ? value : plan.value.toFixed(2).replace('.', ''),
            },
          ],
        }
  ...
  static async createPaghiperToInvoice(invoiceId) {
    try {
      ...
      const data = {
        ...
        type_bank_slip: 'boletoA4',
        days_due_date: 1,
        seller_description: `Referente a ${invoice.itens.map((item) => `${item.name}; `)}`,
        payer_phone: user.whatsapp.replace(' ', '').replace('-', ''),
        notification_url: Env.get(user.controls.migrated_at ? 'PAGHIPER_RETURN_NEXT' : 'PAGHIPER_RETURN'),
      ...
      newSystemRequest.transactionId = paghiper.data['create_request']['transaction_id']
      ...
      for (let i = parseInt(initial); i < parseInt(final); i++) {
        ...
        if (invoices.rows.length > 1) {
          if (
            ...
            invoices.rows.filter((inv) => inv.status !== 'canceled' && inv.status !== 'pending').length > 0
          ) {
            problems.push({
              user: user.id,
              profile: profile.id,
    ...
    try {
      ...
      if (user.controls.type === 'adm') {
        ...
        if (!invoiceUser.controls.paymentInfo) {
          ...
          lastStatus = Object(lastStatus).hasOwnProperty('create_request') ? lastStatus.create_request : lastStatus
          ...
          const { data: result } = await axios.post(`${Env.get('APP_URL')}/paghiper`, lastStatus)
          return response.json({ ...result })
        } else {
          ...
          if (invoice) {
            ...
            try {
              if (invoiceUser.controls.paymentInfo.subscription) {
                await gatewayPagarme.cancelSubscription({}, invoiceUser.controls.paymentInfo.subscription.id)
              }
            } catch (error) {
            ...
            if (invoiceUser.controls.period === 'yearly') {
              invoiceUser.controls.nextInvoice = DateTime.local().plus({ years: 1 }).toFormat('yyyy-MM-dd')
            }
  ...
  async updateDue({ response }) {
    try {
      ...
      for (let user of users.rows) {
        let due = `${moment().format('YYYY-MM-')}${user.due < 10 ? `0${user.due}` : user.due}`
  ...
  async todayPayments({ response, view, session }) {
    try {
      ...
      const invoices = await SystemRequest.query().where('created_at', 'like', moment().format('YYYY-MM-DD%')).fetch()
      response.send(
  ...
  static async allInvoicesOfPayment(invoice) {
    try {
      ...
      return await SystemRequest.query().where({ userId: invoice.userId, expiration: invoice.expiration }).fetch()
    } catch (error) {
      console.error(error)
      throw error
    }
  ...
  async fixDatePayment({ response }) {
    try {
      ...
      let page = 1
      let systemRequests = (await SystemRequest.query().paginate(page, 2000)).toJSON()
      ...
      do {
        ...
        for (let sr of systemRequests.data) {
          ...
          if (Array.isArray(sr.paghiper)) {
            if (sr.paghiper[sr.paghiper.length - 1] && sr.paghiper[sr.paghiper.length - 1].dataStatus) {
              await Database.table('system_requests')
                .where('id', sr.id)
                .update('updated_at', sr.paghiper[sr.paghiper.length - 1].dataStatus)
              ...
              await Database.table('system_requests')
                .where('id', sr.id)
                .update('updated_at', sr.paghiper[0].create_request.due_date ? `${sr.paghiper[0].create_request.due_date} 00:00:00` : sr.expiration)
              await Database.table('invoices')
                .where('id', sr.invoiceId)
                .update('updated_at', sr.paghiper[0].create_request.due_date ? `${sr.paghiper[0].create_request.due_date} 00:00:00` : sr.expiration)
            }
          } else {
            if (sr.paghiper.create_request && sr.paghiper.create_request.due_date) {
              await Database.table('system_requests').where('id', sr.id).update('updated_at', `${sr.paghiper.create_request.due_date} 00:00:00`)
              await Database.table('invoices').where('id', sr.invoiceId).update('updated_at', `${sr.paghiper.create_request.due_date} 00:00:00`)
            } else {
              await Database.table('system_requests').where('id', sr.id).update('updated_at', sr.expiration)
              await Database.table('invoices').where('id', sr.invoiceId).update('updated_at', sr.expiration)
            }
          }
        }
        systemRequests = (await SystemRequest.query().paginate(++page, 2000)).toJSON()
  ...
  static getDataItemsAndValue(plans, userPlans, products, user) {
    ...
    for (const plan of plansArr) {
      const userPlan = userPlansArr.find((uPlan) => uPlan.flexPlanId === plan.id)
      const product = productsArr.find((product) => {
        if (product.systemProductId) {
          return product.id === userPlan.systemProductId
        } else {
          return product.plan_id === userPlan.flexPlanId && product.operations.type === period
        }
      })
