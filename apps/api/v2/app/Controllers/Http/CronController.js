...
class CronController {
      ...
      const list = await User.query()
        ...
        .whereRaw(
          `date(users.created_at) = date(system_requests.created_at) and system_requests.expiration = '${moment()
            ...
            .format(
              "YYYY-MM-DD"
            )}' and system_requests.status in ('pending', 'canceled')`
        )
        .fetch();

      for (let item of list.rows) {
        const user = await User.find(item.userId);
        const systemRequest = await user
          .requests()
          .where("id", item.id)
          .first();
  ...
  async blockProfile({ response }) {
    try {
      ...
      const blockList = await SystemRequest.query()
        .where("limit", moment().format("YYYY-MM-DD"))
      ...
      if (blockList) {
        for (let invoice of blockList.rows) {
          ...
          if (
            (invoice.paghiper &&
              invoice.paghiper[invoice.paghiper.length - 1].status ===
                "canceled") ||
            (invoice.paghiper &&
              invoice.paghiper[invoice.paghiper.length - 1].status ===
                "refunded")
            ...
            if (
              ...
              profile &&
              (fature.type === "monthly" || fature.type === "first")
            ) {
              profile.status = 0;
              invoice.blocked_at = moment().format();
              profile.save();

              const bonusSupport = await BonusSupport.query()
                .where("invoiceId", checkPayment.id)
                .first();
              const firstBonusSupport = await Invoice.query()
                .where({ userId: user.id, type: "monthly" })
                .first();

              if (
                firstBonusSupport &&
                checkPayment.id === firstBonusSupport.id
              ) {
                if (bonusSupport) {
    ...
    try {
      ...
      users.rows.forEach(async (user) => {
        if (user.controls.migrated_at) {
          console.log(
            `Usuário ${user.id} já foi migrado em ${user.controls.migrated_at}`
          );
          return;
        }
        ...
        try {
          ...
          if (doubleProfile) {
            for (const profile of doubleProfile.rows) {
              ...
              const lastBartenderNewDB = await Database.connection("mysql2")
                .select(Database.raw("MAX(id) as max_id"))
                .from("bartenders");

              const lastBartenderOldDB = await Database.connection("mysql")
                .select(Database.raw("MAX(id) as max_id"))
                .from("bartenders");

              const nextId =
                lastBartenderOldDB[0].max_id > lastBartenderNewDB[0].max_id
                  ? lastBartenderOldDB[0].max_id
                  : lastBartenderNewDB[0].max_id;
              ...
              await newConnectionDatabase
                .table("bartenders")
                .insert(newBartenderPerfil);
              ...
              const createProductsAndPizza = async (modelCategory) => {
                ...
                //ProductComplements
                const createProductComplements = async (
                  modelComplement,
                  modelProduct
                ) => {
                  await this.migrateTable({
                ...
                const createComplement = async (modelProduct) => {
                  await this.migrateTable({
                    ...
                    connection: newConnectionDatabase,
                    callbackAfterAnyElement: (modelComplement) =>
                      createProductComplements(modelComplement, modelProduct),
              ...
              await this.migrateTable({
                ...
                callbackAfterLastElement: async () => {
                  ...
                  totalCarts = await this.migrateCarts(
                    profile,
          ...
          console.log({
            ...
            await Mail.raw(
              ...
              ${
                ...
                <li>Duração: ${duration.toFormat(
                  "mm"
                )} minutos e ${duration.toFormat("ss")} segundos</li>
              </ul>
              `,
              (message) => {
                message
                  ...
                  .subject(
                    `Migração de Usuários - ${DateTime.local().toFormat(
                      "dd/MM/yyyy HH:mm"
                    )}`
                  );
              }
          ...
          migrateds.push(
            `${user.id}: ${error.code ? error.code : error.status}`
          );
          console.log(error);
            ...
            await Mail.raw(
              ...
              (message) => {
                message
                  ...
                  .to("jasonaries@gmail.com")
                  .subject(
                    `Migração de Usuários [ ERROR ]- ${DateTime.local().toFormat(
                      "dd/MM/yyyy HH:mm"
                    )}`
                  );
              }
    ...
    do {
      requests = await profile
        ...
        .whereBetween("created_at", [
          fiveDaysAgo.toFormat("yyyy-MM-dd HH:mm:ss"),
          dateNow.toFormat("yyyy-MM-dd HH:mm:ss"),
        ])
      ...
      for (const req of requests.rows) {
        ...
        let requestDuplicate = duplicateRequests.find(
          (r) => r && r.code === req.code
        );
        ...
        const cart = await this.generateCart(
          request,
          profile,
          newConnectionDatabase
        );
  ...
  async migrateTable({
    ...
    try {
      const data = !all
        ? where
          ? await model.query().where(where).fetch()
          : await model[relationName]().fetch()
        : await model.all();
      ...
      if (data.rows) {
        ...
        for (const dt of data.rows) {
          ...
          Logger.transport("file").info(
            `Tabela ${insertTableName} id ${dt.id}`
          );

          if (data.rows.length - index === 1) {
            if (
              callbackAfterLastElement &&
              typeof callbackAfterLastElement === "function"
            ) {
  ...
  async insertInTable(
    ...
    if (verifyExistsFromProperty) {
      const exists = await connection
        ...
        .where(
          verifyExistsFromProperty,
          objectDeserialized[verifyExistsFromProperty]
        )
        .first();
      if (exists) {
  ...
  async generateCart(request, profile, newConnectionDatabase) {
    try {
      ...
      do {
        ...
        cartsAlreadyExists = await newConnectionDatabase
          .table("carts")
          .where({ profileId: profile.id, code: newCart.code })
          .first();
      ...
      newCart = await newConnectionDatabase
        .table("carts")
        .where("id", cartId)
        .first();
  ...
  async generateCartItens(
    { cart, cartPizza, cartId, valueType, categories },
    newConnectionDatabase
  ) {
    try {
      ...
      const pizzas = categories
        .map((category) => category.pizzaProduct)
        .filter((pizza) => pizza);

      cartPizza = cartPizza.map((pizza) => {
        const pizzaProfile = pizzas.find((pizzaProfile) => {
          return (
            pizza.flavors.some((flavor) =>
              pizzaProfile.flavors.some((f) => f.code === flavor.code)
        ...
        return {
          ...
          name: pizzaProfile
            ? `Pizza ${pizza.size} ${pizza.flavors.length} Sabor${
                pizza.flavors.length > 1 ? "es" : ""
              } ${flavors} ${
                pizza.implementations.length
                  ? "com " + pizza.implementations[0].name
                  : ""
              }`
            : "-",
          type: "pizza",
  ...
  async generateClient(
    { clientSearchData, name, date_last_request },
    newConnectionDatabase
  ) {
    try {
      let client = await newConnectionDatabase
        .table("clients")
        .where(clientSearchData)
        .first();
      if (!client) {
        ...
        client = await newConnectionDatabase
          .table("clients")
          .where("id", clientId)
          .first();
      } else {
  ...
  async generateClientAddress(
    { deliveryAddress, clientId, uf },
    newConnectionDatabase
  ) {
    try {
  ...
  async generateFormPayment({
    ...
    try {
      ...
      const profileFormPayment = profileFormsPayment.find(
        (f) => f.label === label
      );
  ...
  getProductFinalValue(product, valueType) {
    ...
    const promoteStatusMenu =
      valueType === "T" ? "promoteStatusTable" : "promoteStatus";
    const promoteValueMenu =
      valueType === "T" ? "promoteValueTable" : "promoteValue";

    return product[promoteStatusMenu]
      ? product[promoteValueMenu]
      : product[valueMenu];
  }
    ...
    if (columnsDateConvert) {
      ...
      columns.forEach((colum) => {
        if (data[colum] && typeof data[colum] === "string") {
          ...
          data[colum] = DateTime.fromISO(newDeletedAt)
