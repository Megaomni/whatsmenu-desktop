    ...
    try {
      ...
      const newUserPlans = await Database.transaction(async (trx) => {
        ...
        const systemProducts = await SystemProduct.query()
          .whereIn('id', [...addProductsIds, ...delProductsIds])
          .fetch()
        ...
        if (user.controls.paymentInfo && user.controls.paymentInfo.gateway && user.controls.paymentInfo.subscription) {
          ...
          switch (gateway) {
            case 'pagarme':
              ...
              const productItemsId = subscription.items.filter((item) => item.id.includes('plan')).map((item) => item.id)
