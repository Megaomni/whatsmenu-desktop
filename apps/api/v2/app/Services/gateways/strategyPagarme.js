...
class StrategyPagarme {
  ...
  async webhookDeletedCard(event) {
    ...
    if (user) {
      console.log('Starting: ', {
        webhook: 'pagarme:card.updated',
        linha: 282,
        metodo: `webhookDeletedCard:${user.id}`,
        data: DateTime.local().toFormat('yyyy-MM-dd HH:mm:ss'),
      })
  ...
  async webhookExpiredCard(event) {
    ...
    if (user) {
      console.log('Starting: ', {
        webhook: 'pagarme:card.expired',
        linha: 297,
        metodo: `webhookExpiredCard:${user.id}`,
        data: DateTime.local().toFormat('yyyy-MM-dd HH:mm:ss'),
      })
  ...
  async webhookUpdatedCard(event) {
    ...
    if (user && user.controls.paymentInfo.cards) {
      console.log('Starting: ', {
        webhook: 'pagarme:card.updated',
        linha: 315,
        metodo: `webhookUpdatedCard:${user.id}`,
        data: DateTime.local().toFormat('yyyy-MM-dd HH:mm:ss'),
      })
      for (const card of user.controls.paymentInfo.cards) {
        if (card.id === data.id) {
  ...
  async webhookChargePaid(event) {
    ...
    if (data.order && data.order.status === 'paid' && data.order.metadata.invoices) {
      ...
      if (user) {
        console.log('Starting: ', {
          webhook: 'pagarme:charge.paid',
          linha: 337,
          metodo: `webhookChargePaid:${user.id}`,
          data: DateTime.local().toFormat('yyyy-MM-dd HH:mm:ss'),
        })
        ...
        if (invoices.rows.length) {
          for (let i = 0; i < invoices.rows.length; i++) {
            ...
            if (invoice.type === 'monthly' && subscription.status !== 'active') {
              const invItems = invoice.itens
                .filter((ivItem) => ivItem.service === 'plan')
                .map((item) => {
  ...
  async webhookChargePaymentFailed(event) {
    ...
    if (data.order && data.order.status === 'failed' && data.order.metadata.invoices) {
      ...
      if (user) {
        console.log('Starting: ', {
          webhook: 'pagarme:charge.payment_failed',
          linha: 421,
          metodo: `webhookChargePaymentFailed:${user.id}`,
          data: DateTime.local().toFormat('yyyy-MM-dd HH:mm:ss'),
        })
        const profile = await user.profile().fetch()
  ...
  async webhookCanceledInvoice(event) {
    ...
    if (user) {
      console.log('Starting: ', {
        webhook: 'pagarme:invoice.canceled',
        linha: 421,
        metodo: `webhookCanceledInvoice:${user.id}`,
        data: DateTime.local().toFormat('yyyy-MM-dd HH:mm:ss'),
      })
  ...
  async webhookInvoiceCreated(event) {
    try {
      ...
      const user = await User.find(data.customer.code)
      console.log('Starting: ', {
        webhook: 'pagarme:invoice.created',
        linha: 469,
        metodo: `webhookInvoiceCreated:${data.customer.code}`,
  ...
  async webhookInvoicePaid(event) {
    ...
    const user = await User.find(data.customer.code)
    console.log('Starting: ', {
      webhook: 'pagarme:invoice.paid',
      linha: 516,
      metodo: `webhookInvoicePaid:${data.customer.code}`,
      user: user ? user.id : null,
      data: DateTime.local().toFormat('yyyy-MM-dd HH:mm:ss'),
    })
    ...
    try {
      if (user && user.controls.paymentInfo) {
        ...
        const invoice = await user
          .invoices()
          .where({ invoice_code: data.id })
          .orWhere({ type: data.charge.recurrence_cycle === 'first' ? 'first' : 'monthly', status: 'pending' })
          .last()
        ...
        if (invoice) {
          ...
          const pendingInvoice = await user
            .invoices()
  ...
  async webhookFailedInvoicePayment(event) {
    ...
    if (user) {
      console.log('Starting: ', {
        webhook: 'pagarme:invoice.payment_failed',
        linha: 510,
        metodo: `webhookFailedInvoicePayment:${user.id}`,
        data: DateTime.local().toFormat('yyyy-MM-dd HH:mm:ss'),
      })
      ...
      if (user.controls.paymentInfo.subscription) {
        if (user.controls.paymentInfo.subscription.status !== 'canceled') {
          const { data: canceledSubscription } = await gatewayPagarme.cancelSubscription(user.controls.paymentInfo.subscription.id, {
  ...
  async webhookUpdatedInvoice(event) {
    ...
    if (user) {
      console.log('Starting: ', {
        webhook: 'pagarme:invoice.updated',
        linha: 653,
        metodo: `webhookUpdatedInvoice:${user.id}`,
        data: DateTime.local().toFormat('yyyy-MM-dd HH:mm:ss'),
      })
      const flexPlans = await FlexPlan.all()
  ...
  async webhookSubscriptionCreated(event) {
    ...
    if (user) {
      console.log('Starting: ', {
        webhook: 'pagarme:subscription.created',
        linha: 688,
        metodo: `webhookSubscriptionCreated:${user.id}`,
        data: DateTime.local().toFormat('yyyy-MM-dd HH:mm:ss'),
      })
      const profile = await user.profile().fetch()
  ...
  async webhookSubscriptionCanceled(event) {
    ...
    if (user) {
      console.log('Starting: ', {
        webhook: 'pagarme:subscription.canceled',
        linha: 724,
        metodo: `webhookSubscriptionCanceled:${user.id}`,
        data: DateTime.local().toFormat('yyyy-MM-dd HH:mm:ss'),
      })
      const profile = await user.profile().fetch()
      user.controls.paymentInfo.subscription.status = 'canceled'
...
const gatewayPagarme = new Gateway(new StrategyPagarme(Env.get('PAGARME_SECURITY_KEY'), Env.get('PAGARME_PUBLIC_KEY')))
