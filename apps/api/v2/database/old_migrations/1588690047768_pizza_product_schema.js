...
class PizzaProductSchema extends Schema {
  up () {
    this.create('pizza_products', (table) => {
      ...
      table.json('sizes').notNullable().comment(JSON.stringify([{name: 'default', flavors: [1, 2, 3], status: true}]))
      table.json('flavors').notNullable().comment(JSON.stringify([
        {
          name: 'flavorName',
      ...
      table.json('implementations').notNullable().comment(JSON.stringify([{name: 'Borda de catupiry', value: 3}, {name: 'Borda Tradicional', value: 0}]))
      table.timestamps()
    })
  }
