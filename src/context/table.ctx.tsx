...
import { setCurrentCommandIdAction, setCurrentTableIdAction, setTablesAction, switchTablesAction, updateCommandAction, updateTableAction } from "../reducers/tables/actions";
...
export function TablesProvider({ children }: TablesProviderProps) {
  ...
  const { profile, handleShowToast, setRequestsToPrint, plansCategory, socketCommands, setSocketCommands, finishCommand, onOnlineCallback, user } =
      ...
      if (currentCommand.haveCarts() && !currentCommand.fullPaid()) {
        ...
        try {
          const { data }: { data: CommandType } = await apiRoute(
            ...
            session,
            "PATCH",
            { fees: currentCommand.fees, formsPayment: currentCommand.formsPayment, tableId: table?.id }
          );
  ...
  const haveFees = !!(profile?.fees?.filter(f => f.status).length || (typeModal === "command" ? command?.fees.length : (table?.opened && table?.opened.getUpdatedFees(false, true).length)))
