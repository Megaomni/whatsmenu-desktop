...
export default function Account({
  ...
  useEffect(() => {
    if (expired && token) {
      handleShowToast({
        ...
        content: "Token de recuperação de senha expirado ou ínvalido",
      });
    }
  ...
  const handleUpdate = async (type: "security" | "access") => {
    ...
    try {
      ...
      handleShowToast({
        type: "success",
        content: `Senha de ${type === "access" ? "acesso" : "segurança"} alterada com sucesso`,
      });
    } catch (error) {
      handleShowToast({
        type: "erro",
  ...
  return (
    <>
      <Title
        title="Meu Perfil"
        componentTitle="Configurações de senhas"
        className="mb-4"
        child={["Senhas"]}
      />
      {expired ? (
          ...
          {profile.not_security_key ?
            (
              <Alert variant="warning" show={true}>
                ...
                <p className="ms-4">Cadastre uma senha financeira</p>
              </Alert>
            ) : null}
          <Card>
                ...
                <Row className="align-items-baseline">
                  <Col md="2">
                    <Form.Label className="fw-bold text-nowrap">
                      {!profile.not_security_key ? "Senha Atual" : "Senha"}
                    </Form.Label>
                  </Col>
                  <Col md="4">
                    <div>
        ...
        <Card>
          <Card.Header className="text-dark">
            <h4>Recuperar Senha Financeira</h4>
          </Card.Header>
          <Card.Body>
            <Container fluid className="mx-0 px-0">
              <Row className="align-items-baseline">
