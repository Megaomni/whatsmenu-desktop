    ...
    async function middleware(req: NextRequest, res: NextResponse) {
        ...
        try {
            ...
            if (cookieValue && req.url.includes(cookieValue as string)) {
                //@ts-ignore
                return NextResponse.redirect(new URL('/dashboard/profile', req.url)).cookie("stripe.success.checkout", "");
            } else if (session && session.user?.controls?.paymentInfo && !req.url.includes("/dashboard/invoices")) {
