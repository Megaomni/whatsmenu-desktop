...
const Update = ({ createdAt, description, upgrades }: IUpdate) => {
  ...
  const Upgrade = ({ plan }: { plan: string }) => {
    ...
    return (
      <>
        ...
        {upgrades.filter(upgrade => upgrade.plan === plan).map(upgrade => {
          let badge = null
          switch (upgrade.type) {
            case "new":
          ...
          return (
            <div key={hash()} className="ms-4" >
              {upgrade.name ? <h3>{upgrade.name} {badge}</h3> : null}
              <ul>
  ...
  return (
    <div className="p-4 mb-2 border">
      <h2>Atualização {DateTime.fromISO(createdAt).toFormat('dd/MM/yyyy')}</h2>
      <p>{description}</p>
      {plans.map(plan => {
        return (
          <Upgrade key={plan} plan={plan} />
        )
      })}
    </div>
  )
