...
import { apiRoute, handleCopy, normalizeCaracter } from "../../../utils/wm-functions";
...
export default function AdmClient({ plans, ...props }: AdmClientProps) {
  ...
  const handleGetUser = async () => {
    ...
    try {
      ...
      console.error(error);
      handleShowToast({ show: true, type: "erro", content: (error as any).response.status === 404 ? "Cliente não encontrado." : "" })
    }
  };
    ...
    <Row>
      <Col>
        ...
        >
          ...
          <tbody>
            {users.data.map((user) => {
              const created_at = DateTime.fromSQL(user.created_at).toFormat("dd-MM-yyyy HH:mm:ss")
              ...
              if (searchValue.trim() !== "") {
                switch (usedFilter.value) {
                  case "all":
                    const valuesArr = [user.id, user.name, user.email,user.whatsapp.replace(/[^\w\s]/gi, '').split(" ").join(""), user.profile?.slug, user.support?.name, user.seller?.name, created_at].filter(el => el);
                    if(!valuesArr.some(text => normalizeCaracter(String(text)).includes(normalizeCaracter(searchValue)))){
                      return
                    }
                    break;
                  case "seller":
                  case "support":
                    if (!(normalizeCaracter(user[usedFilter.value]?.name || "").includes(normalizeCaracter(searchValue)))) {
                      return
                    }
                    break;
                  case "date":
                    if (!(normalizeCaracter(created_at).includes(normalizeCaracter(searchValue)))) {
                      return
                    }
                    break;
                  case "slug":
                    if (!(normalizeCaracter(user.profile?.slug || "").includes(normalizeCaracter(searchValue)))) {
                      return
                    }
                    break;
                  case "whatsapp":
                    const wpp = user.whatsapp.replace(/[^\w\s]/gi, '').split(" ").join("");
                    if (!(normalizeCaracter(wpp).includes(normalizeCaracter(searchValue)))) {
                      return
                    }

                    break;
                  default:
                    if (!(normalizeCaracter(user[usedFilter.value as string]).includes(normalizeCaracter(searchValue)))) {
                      return
                    }
                    break;
                }
              ...
              return (
                ...
                >
                  ...
                  <td className="fs-8 cursor-pointer" onClick={e => { e.stopPropagation(); handleCopy(e, handleShowToast) }}><FaCopy color="#e2e2e2" /> {user.email}</td>
                  <td className="fs-8 cursor-pointer" onClick={e => { e.stopPropagation(); handleCopy(e, handleShowToast) }}><FaCopy color="#e2e2e2" /> {user.whatsapp.replace(/[^\w\s]/gi, '').split(" ").join("")}</td>
                  <td className="fs-8 cursor-pointer" onClick={e => { e.stopPropagation(); handleCopy(e, handleShowToast) }}><FaCopy color="#e2e2e2" /> {user.profile?.slug || "-"}</td>
          ...
          <Card>
            <Card.Body>
              ...
              >
                ...
                <Col sm="4">
                  <Form.Group>
                    <Form.Label className="fs-7 mt-2 mt-md-0">Buscar por</Form.Label>
                    <Form.Select
                      value={search.type}
                      onChange={(e) =>
                ...
                <Col sm="2" className="mt-2 mt-md-0 d-flex align-items-end">
                  <Button variant="success" className="w-100" onClick={handleGetUser}>
                    <span>Buscar</span>
                  </Button>
                </Col>
              </Row>
                ...
                <Row>
                  {showPagination ? (
                    <>
                      <Col md>
                        <span>
                          Mostrando {onlyCard ? users.data.filter((d: any) => d.controls.disableInvoice)?.length : users.data?.length} de {users.total}
                        </span>
                      </Col>
                      <Col md>
                    ...
                    <span>{users.data?.length} {users.data?.length > 1 ? "usuários encontrados" : "usuário encontrado"}</span>
                  )}
                </Row>
...
export const getServerSideProps: GetServerSideProps = async ({ req }) => {
  ...
  const { data: systemProducts } = await apiRoute("/api/v2/systemProducts", session);
