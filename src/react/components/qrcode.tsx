...
export const Qrcode = () => {
  ...
  return (
    <>
      {connected ? (
        <div className="text-white bg-teal-500 rounded-full px-6 py-2">
          Conectado!
        </div>
      ) : !loading.status && !qrcode ? (
        <h1 className="font-bold text-2xl">Desconectado</h1>
      ) : (
        <div className="flex items-start justify-center gap-12 mt-6">
          {qrcode && (
              ...
              <li>
                <div className="flex gap-2">
                  ...
                  <span className="flex gap-2 font-bold">
                    Mais opções{" "}
                    <span className="x1rg5ohu x16dsc37">
            ...
            <div className="flex flex-row gap-4 items-center">
              ...
              {loading.message && !qrcode && (
                <div className="mt-8">
                  <progress
                    id="whatsapp-percent"
                    className="h-3 w-[256px]"
                    max={100}
                    value={loading.percent}
                  ></progress>
                  <p className="text-center">
                    {loading.message} - {loading.percent}%
                  </p>
                </div>
              )}
            </div>
