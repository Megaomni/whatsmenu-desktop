...
export class ClientidComponent implements OnInit, AfterViewChecked {
  ...
  async saveCustomerId(type: 'update' | 'create') {
    ...
    if (this.customer.birthday_date) {
      this.customer.birthday_date = DateTime.fromFormat(this.customer.birthday_date, 'dd-MM-yyyy').toFormat('yyyy-MM-dd HH:mm:ss') 
    }
    ...
    this.dialogRef.close({ targetModal: this.data.targetModal ? this.data.targetModal : this.data.deliveryButton ? 'listAddress': 'formPayment', customerInfo })
  }

  async getClientByWhatsapp(event?: KeyboardEvent) {
    if (!this.hasFetchedCostumer) {
      if ((event && event.key !== 'Enter') || this.customer.whatsapp.length < 11) {
        return
      }
      try {
  ...
  public formatToDatePicker(date: string, format: 'fromSQL' | 'fromISO' | 'dd-MM-yyyy') {
    ...
    return formatedDate.toFormat('dd-MM-yyyy')
  }
