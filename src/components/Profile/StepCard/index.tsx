...
import { currency, getProductAndPrice } from '../../../utils/wm-functions'
...
export function PaymentCard({ invoices, systemProducts, plans }: { invoices: Invoice[]; systemProducts: SystemProduct[]; plans: Plan[] }) {
  ...
  const { handleConfirmModal, handleShowToast, gateway, user, dispatchUser } = useContext(AppContext)
  ...
  const changeCharge = async (card_id: string, installments: number = 1): Promise<any> => {
    try {
          ...
          case 'pagarme':
            try {
              ...
              if (newPurchasePagarmeCard.last_transaction.status === 'with_error') {
                throw new Error('Houve uma falha na transação')
              } else {
      ...
      handleShowToast({
        title: 'Alterar Cartão',
        content: 'Não foi possível alterar o cartão de cobrança',
      })
    }
  }
      ...
      switch (userGateway) {
        ...
        case 'pagarme':
          try {
            if (user?.controls?.paymentInfo?.subscription?.id) {
              ...
              handleShowToast({
                title: 'Checkout',
                content: 'Redirecionando para Pagina de Pagamento',
              })
            ...
            handleShowToast({
              title: 'Checkout',
              content: 'Estamos com dificuldade para gerar a tela de pagamento, tente novamente',
            })
          }
        ...
        default:
          handleShowToast({
            ...
            content: 'Não encontramos nenhum gateway de pagamento',
          })
      }
    }
  }
      ...
      switch (userGateway) {
        case 'pagarme':
          try {
            handleShowToast({
              title: 'Assinatura',
              content: 'Estamos adicionando sua assinatura.',
            })
            ...
            if (!user.controls.paymentInfo) {
              handleShowToast({
                title: 'Assinatura',
                content: 'Houve uma falha ao adicionar sua assinatura',
              })
            ...
            handleShowToast({
              title: 'Assinatura',
              content: 'Sua assinatura foi criada com sucesso',
              type: 'success',
            })
            ...
            handleShowToast({
              title: 'Assinatura',
              content: 'Houve uma falha no processamento tente novamente',
              delay: 10000,
            })
          }
          break
  ...
  const disabledPayButtonInvoice = (invoice: Invoice) => {
    if (invoicesId.has(invoice.id) && invoices.some((inv) => inv.type !== invoice.type)) {
      return {
        title: 'Existe uma fatura mensal em andamento.',
        disabled: true,
      }
    }
    ...
    if (user?.controls?.paymentInfo?.subscription?.status === 'canceled' && invoice.type === 'addon') {
      return {
        title: 'Sua assinatura esta cancelada',
        disabled: true,
      }
    }
  }
    ...
    try {
      ...
      if (payment.status !== 'paid') {
        ...
        if (last_transaction && last_transaction.status === 'not_authorized') {
          ...
          handleShowToast({
            title: 'Pagamento',
            content: 'Houve uma falha no processamento do pagamento verifique os dados e certifique-se que existe limite disponível em seu cartão',
            delay: 3000,
          })
          return
        }
      ...
      handleShowToast({
        title: 'Pagamento',
        content: 'Pagamento efetuado com sucesso, iremos recarregar a pagina.',
        type: 'success',
        delay: 4000,
      })
  ...
  return (
    <>
      <Card>
        <Card.Header className="fw-bold fs-5">Faturas em Aberto</Card.Header>
        <Card.Body>
          <Table responsive>
            <thead>
              <tr>
              ...
              {invoices.map((invoice) => {
                return (
                  <tr key={invoice.itens.toString()} style={{ backgroundColor: invoicesId.has(invoice.id) ? 'rgba(0, 0, 0, .1' : '' }}>
                    ...
                    <td>
                      <span>{currency({ value: invoice.value, currency: user?.controls?.currency })}</span>
                    </td>
                    <td>
                    ...
                    <td>
                      ...
                      >
                        {invoicesId.has(invoice.id) ? 'Em Andamento' : 'Pagar'}
                      </Button>
                    </td>
                  </tr>
                )
      ...
      <Card>
        <Card.Header className="fw-bold fs-5">Carrinho - {cart.length ? 'Confira os itens' : 'Não há itens no momento'}</Card.Header>
        {cart.length ? (
          <>
            <Card.Body>
              <Row>
                        ...
                        <div className="d-flex flex-column align-items-center flex-grow-1 justify-content-between">
                          ...
                          <div className="text-center">
                            <span className="fs-7" style={{ textDecoration: product.quantityDiscount ? 'line-through' : '' }}>
                              {product.quantity}X {currency({ value: unitValue, currency: user?.controls?.currency })}
                            </span>
                            <br />
                            <span className="fs-6 fw-bold">{currency({ value, currency: user?.controls?.currency })}</span>
                            {product.service !== 'plan' ? <span>/Única</span> : <span>/{user?.controls?.period === 'monthly' ? 'Mês' : 'Ano'}</span>}
                          </div>
                        </div>
                      </Col>
                    )
              ...
              <Row className="mt-3">
                <Col>
                  <p>
                    Olá, <b>{user?.name}</b> após a confirmação de pagamento você será redirecionado a página será recarregada.
                  </p>
                </Col>
              </Row>
            </Card.Body>
            <Card.Footer className="d-flex gap-2 flex-column flex-md-row">
              <Button onClick={getPaymentMethod}>Continuar para Pagamento</Button>
              {/* {
