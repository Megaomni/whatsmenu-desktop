...
import { Accordion, Button, Card, Modal, Form, Row, Col, Container } from 'react-bootstrap'
...
export function ProfileFormsPayment({ isActive = false }: ProfileFormsPaymentProps) {
  ...
  const onlineCredit = profileState.formsPayment.find((f) => f.payment === 'credit')
  const onlinePix = profileState.formsPayment.find((f) => f.payment === 'pix')

  return (
    <>
      <div className="d-flex justify-content-end my-3 mx-2">
        <HelpVideos.Trigger urls={[{ src: 'https://www.youtube.com/embed/VabVi5WMR8c', title: 'Formas de pagamento' }]} />
      </div>
