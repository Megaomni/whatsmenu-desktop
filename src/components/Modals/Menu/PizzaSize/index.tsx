...
export function PizzaSize(props: PizzaSizeProps) {
  ...
  const flavors = ["1 Sabor", "2 Sabores", "3 Sabores", "4 Sabores"];
  ...
  useEffect(() => {
    ...
    if (size?.covers.length) {
      ...
      const imgs = Array(4).fill("").map((i, index) => i = `/images/pizzas/${index + 1}.jpg`)
      setCovers(imgs)
    }
    ...
    try {
      ...
      if (product) {
        if (type === 'create') {
          ...
          if (sizeExists) {
            handleShowToast({
              ...
              type: 'alert',
              title: "Tamanho",
              content: `Já existe um tamanho com este nome "${name}".`
            })
        ...
        handleShowToast({
          show: true,
          title: "Tamanho",
          content: `${size.name?.trim() || name}, foi ${type == "create" ? "criado" : "atualizado"} com sucesso.`,
          type: 'success'
        })
        handleClose();
      }
      ...
      handleShowToast({
        ...
        type: 'erro',
        title: "Tamanho",
        content: `Não foi possível ${type === "create" ? "criar" : "atualizar"} o tamanho ${size.name}.`
      })
    }
  }
  ...
  const deleteSize = async () => {
    ...
    try {
      if (product) {
        if (product.flavors.length && product.sizes.length === 1) {
          handleShowToast({
            show: true,
            title: `Tamanho`,
            content: `Para excluir é necessário apagar os sabores ou adicionar outro tamanho de pizza.`,
            type: 'alert'
          });
        ...
        handleConfirmModal({
          ...
          actionConfirm: async () => {
            ...
            handleShowToast({
              show: true,
              title: "Tamanho",
              content: `${size.name}, foi deletado com sucesso.`,
              type: 'success'
            });
      ...
      handleShowToast({
        show: true,
        title: 'Tamnho',
        content: `Não foi possível excluír o tamanho ${size.name}.`,
        type: 'erro'
      })
    }
  }
    ...
    try {
      if (product) {
        ...
        handleShowToast({
          ...
          type: 'success',
          title: 'Tamanho',
          content: `${size.name}, foi ${size.status ? "pausado" : "despausado"} com sucesso.`
        });
      ...
      handleShowToast({
        ...
        type: 'erro',
        title: "Tamanho",
        content: `Não foi possível ${size.status ? "pausar" : "despausar"} o tamanho ${size.name}.`
      })
    } finally {
    ...
    <div onKeyDown={(e) => {
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            {type === "create" ? "Adicionar Tamanho" : `Editar Tamanho (${size.name})`}
          </Modal.Title>
        </Modal.Header>
                    ...
                    <Nav.Item>
                      <Nav.Link eventKey="details" onClick={e => setEventKeysTabs("details")}>
                        Detalhes
                        {
                          ...
                          <span className="ms-2"><BsExclamationCircle className="pulseElement" color="red" size={20} /></span>
                        }
                      </Nav.Link>
                    </Nav.Item>
                    {
                                ...
                                <Row>
                                  <Col sm>
                                    ...
                                    <div className="position-relative">
                                      ...
                                      >
                                        {
                                          (nameInvalid && !name) && "Por favor insira um nome válido!"
                                        }
                                        {
                                          findSize(name) && "Já existe um tamanho com este nome."
                                        }
                                      </Form.Control.Feedback>
                                    </div>
                                  ...
                                  <Col sm className="d-flex flex-column">
                                    ...
                                    <div className="d-flex flex-wrap flex-md-nowrap justify-content-start gap-2">
                                      {flavors.map((flavor, index) => {
                                        ...
                                        return <div
                                          key={`${flavor}`}
                                          className="d-flex flex-row-reverse gap-2 justify-content-end " {...{style: {flex: window.innerWidth < 768 ? "1 0 25%" : ""}}}
                                        >
                                          <Form.Label htmlFor={`${flavor}`}>
                                            <span className="fs-8 text-nowrap fw-bold">
                                              {flavor}
                                            </span>
                                          </Form.Label>
                                          <Form.Check id={`${flavor}`}
                                      ...
                                      <div className="wm-default-border-none text-dark py-5 px-3 position-relative">
                                        <Row className="align-items-center">
                                          ...
                                          <Col sm className="text-600 fs-8 mb-3">
                                            Para pausar as vendas deste item, clique
                                            no botão ao lado. Se o botão estiver
                                            habilitado, o item não aparecerá na sua
                                            lista de pratos.
                                          </Col>
                                        </Row>
                                        <OverlaySpinner
