...
export function Fees(props: FeesProps) {
  ...
  const { handleShowToast, user } = useContext(AppContext)
  ...
  const handleShortcutActions = useCallback((e: KeyboardEvent) => {
  ...
  const handleSaveFees = async () => {
    if (typeModal === "command" && command?.fees.length) {
      try {
        await apiRoute(
          "/dashboard/command/updateFees",
          session,
          "PATCH",
          { commandId: command?.id, fees: command?.fees }
        );
      } catch (error) {
        handleShowToast({ type: "erro", title: "Taxas Comanda" });
      ...
      if (commandsFees.length) {
        try {
          ...
          if (haveFees) {
            handleShowToast({
              type: "success",
              title: `Taxas ${typeModal === "table" ? "mesa" : "comanda"}`,
              position: "top-center",
            });
          }
  ...
  return (
    <Modal show={show} onEnter={() => { !haveFees && handleConfirm() }} className={`${haveFees ? "visible" : "invisible"}`} onHide={() => { handleClose(); }} centered backdrop="static">
      <Modal.Header>
        <h3>
          <b>Taxas</b>
        </h3>
      ...
      <Modal.Body>
        <h4>
          <b>
            {typeModal === "command" ? "Comanda" : "Mesa"}:{" "}
                ...
                <tr key={fee.code} className="text-center align-baseline">
                        ...
                        <div>
                              ...
                              <Popover id="popover-basic">
                                ...
                                >
                                  {table
                                    ?.activeCommands()
                                    ?.map((command, index) => {
                                      const commandFee: ProfileFee | undefined = command.fees.find(
                                        (f) => f.id === fee.id
                                      );
                                      return (
                                        commandFee && (
                          ...
                          >
                            <Button
                              ...
                              style={{ outline: "none" }}
                            >
                              {table?.activeCommands()?.flatMap(command => command.fees).reduce((totalQuantity, feeReduce) => {
                  ...
                  <td className="text-center">
                    {fee.type === "percent"
                      ? `${fee.value}%`
                      : currency({
                        value: typeModal === "command" ? fee.automatic && fee.status ? (fee.quantity as number) * fee.value : 0 : table?.activeCommands()?.flatMap(command => command.fees).reduce((totalQuantity, feeReduce) => {
                  ...
                  <td>
                    <Form.Check
                      checked={typeModal === 'table' ? table?.activeCommands().some(c => c.fees.find(f => f.code === fee.code)?.automatic) : fee.automatic}
                      id="checkAutomatic"
                      onChange={(e) => {
                        if (typeModal === "table" && table) {
      ...
      <Modal.Footer className="d-flex justify-content-between align-items-start">
        <div className="fs-8 fw-bold m-0">
          <p className="m-0">Atalhos</p>
          <p className="m-0">Enter: Próximo/Confirmar</p>
          <p className="m-0">Esc: Voltar/Cancelar</p>
        </div>
