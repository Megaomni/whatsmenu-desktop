...
import { currency, getNow, groupCart, hash } from "../../../../../utils/wm-functions";
...
export function DistribuitionPackage({ carts, onReturnModal, ...props }: PropsType) {
  ...
  const cartsOnlyDate = carts.filter(cart => cart.date().onlyDate === getNow({ format: 'dd/MM/yyyy' }).nowFormat)
  ...
  const $cart = () => {
    return groupedCart.filter(cart => cart.type === "default").map((prod, indexProd, arrProd) => {
      return (
        <>
          <div
  ...
  const $cartPizza = () => {
    return groupedCart?.filter(pizza => pizza.type === "pizza").map((pizza, indexPizza, arrPizza) => {
      return (
        <>
          <div
            ...
            <div className="py-1 w-100">
              ...
              {pizza.details?.implementations.map((implementation) => {
                return (
                  <div
                    key={hash()}
                    className="m-0 my-1 fw-bold fs-8 w-100`"
                  >
                    <Row className="w-100">
    ...
    <>
      ...
      >
        <Modal.Header className="justify-content-center" >
          <Modal.Title >
            <h3 > Distribuição - <span>{getNow({ format: 'dd/MM/yyyy' }).nowFormat}</span></h3>
          </Modal.Title>
        </Modal.Header>
        <Modal.Body className="my-0 py-0" >
          <Row style={{ minHeight: '70vh' }} className="flex-column flex-md-row">
            <Col sm="12" md="6" className="border-end py-2 overflow-auto" style={{ maxHeight: window.innerWidth < 768 ? "30vh" : "unset" }}>
              <h6 className="text-center">Código do Pedido</h6>
              <div>
                <ListGroup
                    ...
                    cartsOnlyDate?.map(cart => {
                      return (
                        <ListGroup.Item
                          ...
                          {`wm${cart.code}-${cart.type} ${cart.client?.name} ${cart.status === "canceled" ? "(Cancelado)" : ""}`}
                        </ListGroup.Item>
                      )
                    })
                  }
            ...
            <Col sm="12" md="6" className="border-end py-2 flex-grow-1 flex-md-grow-0">
              <h6 className="text-center">
                {cartSelected ? `wm${cartSelected.code}-${cartSelected.type} ${cartSelected.status === "canceled" ? "(Cancelado)" : ""}` : 'Nenhum Pedido Selecionado'}
              </h6>
