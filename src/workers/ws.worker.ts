...
self.onmessage = (event) => {
  ...
  port.onmessage = (message: MessageEvent<IWebSocketEventData>) => {
    ...
    if (event === 'connect' || event === 'reconnect') {
      ...
      if (event === 'connect') {
        console.log('%c[ws-connecting]:', 'color: #00ffaf', `creating socket connection on ${data.url}`, ` - ${new Date().toTimeString()}`) 
      }
      if (ws) {
        ws.onopen = (event: any) => {
      ...
      ws.onclose = (event) => {
        if (connection) {
          ...
          if (!noReconnect) {
            attempts++
            port.postMessage({ event: 'reconnect', data: { connection, attempts } })
            console.log('%c[ws-reconnecting]:', 'color: #aaffaf', `tentaive ${attempts} socket reconnection on ${data.url}`, ` - ${new Date().toTimeString()}`)
          }
        }
      }
      ...
      ws.onmessage = (event) => {
        ...
        switch (data.t) {
          case 3: {
            ...
            port.postMessage({ event: 'subscribe', data: { topic: data.d.topic } })
            break;
          }
          case 7: {
            const data = JSON.parse(event.data)
            if (data.d.event.includes('request')) {
              ...
              requests.sort((a, b) => {
                ...
                port.postMessage({ event: 'request', data: { request, topic: data.d.topic } })
              });
            }
            if (data.d.event.includes('command')) {
              const commands: CommandType[] = data.d.data.commandsWs
              commands.forEach(command => {
                port.postMessage({ event: 'command', data: { command, topic: data.d.topic } })
              });
            }
